...
function logError($message) {
    error_log('[Internship Experience Upload] ' . $message);
}
...
// Fetch existing document before processing the upload
try {
...
if (isset($_POST['submit'])) {
    ...
    if (!isset($_FILES["internship_experienceDocument"]) || $_FILES["internship_experienceDocument"]["error"] !== UPLOAD_ERR_OK) {
        logError("File upload error: " . ($_FILES["internship_experienceDocument"]["error"] ?? 'No file uploaded'));
        echo json_encode([
    ...
    logError("Temporary file path: " . $_FILES["internship_experienceDocument"]["tmp_name"]);
    logError("Upload error code: " . $_FILES["internship_experienceDocument"]["error"]);
    ...
    if ($_FILES["internship_experienceDocument"]["size"] > 10000000) {
        logError("File size exceeds limit: " . $_FILES["internship_experienceDocument"]["size"]);
        echo json_encode([
    ...
    if (move_uploaded_file($_FILES["internship_experienceDocument"]["tmp_name"], $target_file)) {
        ...
        try {
            ...
            $sql = $existing_document ? 
                ...
                "INSERT INTO internship_experience 
                 (student_id, document_name, uploaded_path, upload_date, status) 
                 VALUES (?, ?, ?, NOW(), ?)";
            ...
            if ($stmt->execute($params)) {
                ...
                echo json_encode([
                    'status' => 'Internship Experience uploaded successfully. Awaiting review.',
                    'status_code' => 'success',
                    'alert' => 'Success'
                ]);
            } else {
        ...
        logError("File move failed. Error: " . $_FILES["internship_experienceDocument"]["error"]);
        echo json_encode([
            'status' => 'Upload failed. Error: ' . $_FILES["internship_experienceDocument"]["error"],
            'status_code' => 'error',
            'alert' => 'Error'
        ]);
    }
...
if ($existing_document) {
    $status_msg = [
        ...
        'denied' => "Your Internship Experience was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Internship Experience has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Internship Experience</title>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">Internship Experience Submission</h1>
                </div>

                <div class="template-box">
                    <a href="templates/internship_experience/internship_experience.docx" download="internship_experience.docx" class="btn1-download-template">
                        <img src="images/doc.png" alt="" class="doc-icon"><u>Internship_Experience_Template.docx</u>
                    </a>
                    <a href="templates/internship_experience/internship_experience.docx" download="internship_experience.docx" class="btn2-download-template btn-download">
                    ...
                    <div class="content">
                        <div class="document-area" id="documentArea">
                            <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                                ...
                                ?>
                                    <img id="documentImage" src="<?php echo htmlspecialchars($existing_document['uploaded_path']) . $cache_buster; ?>" alt="Internship Experience Image">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" enctype="multipart-form-data">
                                <input type="file" id="fileInput" name="internship_experienceDocument" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden">
                ...
                editBtn.addEventListener('click', function() {
                    ...
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This Internship Experience has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This Internship Experience has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
                ...
                <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                    ...
                    if (fileExt === 'pdf') {
                        ...
                        printWindow.document.write(`
                            ...
                            <html>
                            <head>
                                <title>Print Internship Experience</title>
                                <style>
