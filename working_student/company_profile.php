...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/company_profile/";
$relative_dir = "/PUP/student/uploads/company_profile/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["company_profileDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["company_profileDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["company_profileDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["company_profileDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if ($_FILES["company_profileDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["company_profileDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["company_profileDocument"]["tmp_name"], $target_file)) {
        ...
        error_log("File move failed. Error: " . $_FILES["company_profileDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["company_profileDocument"]["error"];
        $_SESSION['status-code'] = "error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'company_profile'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Company Profile  has been approved.",
        'denied' => "Your Company Profile was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Company Profile has been submitted and Under review."
    ];
                    ...
                    <div class="content">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="company_profileDocument" accept=".pdf,.doc,.docx" class="hidden">
