...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/supervisor_details/";
$relative_dir = "/PUP/student/uploads/supervisor_details/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["supervisor_detailsDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["supervisor_detailsDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["supervisor_detailsDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["supervisor_detailsDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if (!in_array($mime, $allowed_mimes)) {
        ...
        header("Location: supervisor_details.php");
        exit;
    }

    if ($_FILES["supervisor_detailsDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["supervisor_detailsDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["supervisor_detailsDocument"]["tmp_name"], $target_file)) {
        ...
        error_log("File move failed. Error: " . $_FILES["supervisor_detailsDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["supervisor_detailsDocument"]["error"];
        $_SESSION['status-code'] = "error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'supervisor_details'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Supervisor Details  has been approved.",
        'denied' => "Your Supervisor Details was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Supervisor Details has been submitted and Under review."
    ];
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="title">Supervisor Details</h1>
                </div>
                    ...
                    <div class="content">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="supervisor_detailsDocument" accept=".pdf,.doc,.docx" class="hidden">
