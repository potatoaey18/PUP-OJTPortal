...
if (isset($_POST['submit'])) {
    ...
    error_log("Temporary file path: " . $_FILES["endorsementDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["endorsementDocument"]["error"]);
    ...
    if ($_FILES["endorsementDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["endorsementDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["endorsementDocument"]["tmp_name"], $target_file)) {
        ...
        error_log("File move failed. Error: " . $_FILES["endorsementDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["endorsementDocument"]["error"];
        $_SESSION['status-code'] = "error";
    }
...
if ($existing_document) {
    $status_msg = [
        ...
        'pending' => "Your Endorsement has been submitted and Under review."
    ];
