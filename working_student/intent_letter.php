...
if (isset($_POST['submit'])) {
    ...
    if (!file_exists($target_dir)) {
        if (!mkdir($target_dir, 0755, true)) {
            ...
            echo "<script>window.location.href='intent_letter.php'</script>";
            exit;
        }
    ...
    if (!isset($_FILES["intentDocument"]) || $_FILES["intentDocument"]["error"] !== UPLOAD_ERR_OK) {
        logError("File upload error: " . $_FILES["intentDocument"]["error"]);
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">Intent Letter Document Template</h1>
                </div>
