...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/contract/";
$relative_dir = "/PUP/student/uploads/contract/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["contractDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["contractDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["contractDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["contractDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if ($_FILES["contractDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["contractDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["contractDocument"]["tmp_name"], $target_file)) {
        ...
        error_log("File move failed. Error: " . $_FILES["contractDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["contractDocument"]["error"];
        $_SESSION['status-code'] = "error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'contract'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Contract and Job Description  has been approved.",
        'denied' => "Your Contract and Job Description was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Contract and Job Description has been submitted and Under review."
    ];
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="title">Contract and Job Description</h1>
                </div>
                    ...
                    <div class="content">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="contractDocument" accept=".pdf,.doc,.docx" class="hidden">
