...
// Handle working student status update
if (isset($_POST['mark_working_student'])) {
    try {
        // Ensure PDO connection
        if (!isset($conn) || !($conn instanceof PDO)) {
            error_log("Invalid PDO connection in stud_profile.php");
        ...
        // Update is_working_student to 'yes'
        $stmt = $conn->prepare("UPDATE students_data SET is_working_student = 'no' WHERE id = ?");
        if ($stmt === false) {
            error_log("Prepare failed for student_id $studID: " . $conn->errorInfo()[2]);
            $_SESSION['alert'] = "Error";
        ...
        if ($stmt->execute([$studID])) {
            // Log the action in system_notification
            date_default_timezone_set('Asia/Manila');
            $date = date('F / d l / Y');
            $time = date('g:i A');
            $logs = 'Switched to working student status.';

            $logStmt = $conn->prepare("INSERT INTO system_notification(student_id, logs, logs_date, logs_time) VALUES (?, ?, ?, ?)");
            if ($logStmt === false) {
                error_log("Prepare failed for notification log: " . $conn->errorInfo()[2]);
            } else {
                $logStmt->execute([$studID, $logs, $date, $time]);
            }

            $_SESSION['alert'] = "Success";
            $_SESSION['status'] = "Successfully marked as working student.";
            $_SESSION['status-code'] = "success";
            header("Location: ../dashboard.php");
            exit;
        } else {
            error_log("Update failed for student_id $studID");
            $_SESSION['alert'] = "Error";
            $_SESSION['status'] = "Failed to update working student status.";
            $_SESSION['status-code'] = "error";
            header("Location: stud_profile.php");
            exit;
        }
    } catch (PDOException $e) {
        error_log("Database error for student_id $studID: " . $e->getMessage());
        $_SESSION['alert'] = "Error";
...
<body>
            ...
            <div class="profile-card">
                <div class="profile-content">
                    <div class="profile-image">
                        ...
                        <form action="" method="POST" id="working-student-form" style="display: none;">
                            <input type="hidden" name="mark_working_student" value="1">
                        </form>
                        <div class="switch-ui" onclick="confirmWorkingStudent()">
                            <div class="icon-container">
                                <img src="images/student.png" alt="">
                            </div>
                            <span class="button-text">Present as a Student Trainee</span>
                        </div>
                    </div>
    ...
    <script>
        ...
        function confirmWorkingStudent() {
            swal({
                title: "Switch to Student Trainee?",
                text: "Are you sure you want to present as a Student Trainee?",
                type: "warning",
                ...
                if (isConfirm) {
                    document.getElementById('working-student-form').submit();
                }
            });
        }
    </script>
