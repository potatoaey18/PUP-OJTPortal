...
if (isset($_POST['userUNIQUEid_receiver'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM supervisor WHERE uniqueID = ?");
...
?>
    <div class="py-2 px-4 border-bottom d-none d-lg-block">
        ...
        <div class="position-relative">
            <img src="<?php echo $results['supervisor_profile_picture']; ?>" class="rounded-circle mr-1" width="40" height="40">
        </div>
...
$(document).ready(function() {
    ...
    $("#sendButton").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "send_student_image_to_supervisor.php",
            data: formData,
           ...
           $(function(){
            var receiverId = <?php echo json_encode($receiverId); ?>;
             setInterval(function(){
              $.ajax({
                type: "POST",
                url: "chat_supervisor_load_NewMessage.php", // Create a separate PHP file to check for new messages
    ...
    $("#messageForm").submit(function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "send_studentTOsupervisor_chat.php",
