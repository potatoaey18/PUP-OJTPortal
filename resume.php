...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/resume/";
$relative_dir = "/PUP/student/uploads/resume/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["resumeDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["resumeDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["resumeDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["resumeDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if ($_FILES["resumeDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["resumeDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["resumeDocument"]["tmp_name"], $target_file)) {
        error_log("File successfully moved to: " . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'resume'");
            ...
            if ($stmt->execute($params)) {
                ...
                $_SESSION['status'] = "Resume uploaded successfully. Awaiting review.";
        ...
        error_log("File move failed. Error: " . $_FILES["resumeDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["resumeDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Resume/CV has been approved.",
        'denied' => "Your Resume/CV was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Resume/CV has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Student Resume</title>
                    ...
                    <div class="content">
                        <div class="document-area" id="documentArea">
                            <?php if ($existing_document): ?>
                                ...
                                ?>
                                    <img id="documentImage" src="<?php echo $existing_document['uploaded_path'] . $cache_buster; ?>" alt="Resume Image">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="resumeDocument" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden">
                ...
                editBtn.addEventListener('click', function() {
                    ...
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This resume has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This resume has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
