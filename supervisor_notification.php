...
if(isset($_POST['action'])) {
    $action = $_POST['action'];
    $studentId = $_SESSION['auth_user']['supervisor_id'] ?? null;
...
<head>
    ...
    <title>OJT Web Portal: Notification</title>
    <!-- ================= Favicon ================== -->
    <!-- Standard -->
                ...
                <div class="card-body">
                    <div id="notifications-container" class="notifications-container">
                        ...
                        if(isset($_SESSION['auth_user']['supervisor_id'])) {
                            $studID = $_SESSION['auth_user']['supervisor_id'];
                            
                            // Get all notifications for this student
                            $stmt = $conn->prepare("SELECT supervisor_system_notification.*, students_data.profile_picture 
                                                  FROM supervisor_system_notification 
                                                  LEFT JOIN students_data ON students_data.id = supervisor_system_notification.supervisor_id 
                                                  WHERE supervisor_system_notification.supervisor_id = ? 
                                                  ORDER BY supervisor_system_notification.logs_date DESC, supervisor_system_notification.logs_time DESC");
                            $stmt->execute([$studID]);
