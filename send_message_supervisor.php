...
try {
    ...
    $stmt = $conn->prepare("SELECT user_id FROM users_coordinator_hte WHERE user_id = ?");
    $stmt->execute([$coordinators_id]);
    $user = $stmt->fetch();
    
    if (!$user) {
        try {
            // First try to insert with username if column exists
            $stmt = $conn->prepare("INSERT INTO users_coordinator_hte (user_id, username) VALUES (?, ?)");
            $stmt->execute([$coordinators_id, 'coordinators_' . $coordinators_id]);
        } catch (PDOException $e) {
            // If username column doesn't exist, try without it
            if (strpos($e->getMessage(), 'username') !== false) {
                $stmt = $conn->prepare("INSERT INTO users_coordinator_hte (user_id) VALUES (?)");
                $stmt->execute([$coordinators_id]);
            } else {
    ...
    $stmt = $conn->prepare("
        SELECT id FROM conversations_coordinator_hte
        WHERE coordinator_id = ? AND supervisor_id = ?
    ");
    $stmt->execute([$coordinators_id, $supervisor_id]);
    ...
    if ($conversation) {
        ...
        try {
            ...
            $stmt = $conn->prepare("
                INSERT INTO conversations_coordinator_hte (coordinator_id, supervisor_id, created_at, updated_at)
                VALUES (?, ?, NOW(), NOW())
            ");
            $stmt->execute([$coordinators_id, $supervisor_id]);
        } catch (PDOException $e) {
            ...
            if (strpos($e->getMessage(), 'updated_at') !== false) {
                $stmt = $conn->prepare("
                    INSERT INTO conversations_coordinator_hte (coordinator_id, supervisor_id, created_at)
                    VALUES (?, ?, NOW())
                ");
                $stmt->execute([$coordinators_id, $supervisor_id]);
            } else {
    ...
    try {
        // First try with both created_at and updated_at
        $stmt = $conn->prepare("
            INSERT INTO messages_coordinator_supervisor (conversation_id, sender_id, content, created_at, updated_at)
            VALUES (?, ?, ?, NOW(), NOW())
        ");
        $stmt->execute([$conversation_id, $coordinators_id, $content]);
    } catch (PDOException $e) {
        // If updated_at column doesn't exist, try without it
        if (strpos($e->getMessage(), 'updated_at') !== false) {
            $stmt = $conn->prepare("
                INSERT INTO messages_coordinator_supervisor (conversation_id, sender_id, content, created_at)
                VALUES (?, ?, ?, NOW())
            ");
            $stmt->execute([$conversation_id, $coordinators_id, $content]);
        } else {
    ...
    try {
        $stmt = $conn->prepare("
            ...
            COALESCE(
                (SELECT CONCAT(first_name, ' ', last_name) FROM coordinators_account WHERE id = ?),
                'Unknown Sender'
            ) as sender_name
            FROM messages_coordinator_supervisor m
            WHERE m.id = ?
        ");
        $stmt->execute([$coordinators_id, $message_id]);
        ...
        $response_data = [
            ...
            'sender_id' => $coordinators_id,
            'sender_type' => 'coordinators',
            'content' => $content,
    ...
    if (defined('PUSHER_APP_KEY') && !empty(PUSHER_APP_KEY)) {
        try {
            ...
            // Also trigger to coordinators's channel if needed
            $pusher->trigger(
                'private-coordinators-' . $coordinators_id,
                'message_sent',
                [
                    'status' => 'delivered',
