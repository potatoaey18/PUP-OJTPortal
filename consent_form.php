...
function logError($message) {
    error_log('[Consent Form Upload] ' . $message);
}
...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/consent_form/";
$relative_dir = "/PUP/student/uploads/consent_form/";
...
if (isset($_POST['submit'])) {
    ...
    if (!isset($_FILES["consentFormDocument"]) || $_FILES["consentFormDocument"]["error"] !== UPLOAD_ERR_OK) {
        logError("File upload error: " . ($_FILES["consentFormDocument"]["error"] ?? 'No file uploaded'));
    ...
    $original_name = basename($_FILES["consentFormDocument"]["name"]);
    ...
    logError("Temporary file path: " . $_FILES["consentFormDocument"]["tmp_name"]);
    logError("Upload error code: " . $_FILES["consentFormDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["consentFormDocument"]["tmp_name"]);
    finfo_close($finfo);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if ($_FILES["consentFormDocument"]["size"] > 10000000) {
        logError("File size exceeds limit: " . $_FILES["consentFormDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["consentFormDocument"]["tmp_name"], $target_file)) {
        logError("File successfully moved to: " . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'consent_form'");
            ...
            if ($stmt->execute($params)) {
                ...
                $_SESSION['status'] = "Consent Form uploaded successfully. Awaiting review.";
        ...
        logError("File move failed. Error: " . $_FILES["consentFormDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["consentFormDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'consent_form'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Consent Form has been approved.",
        'denied' => "Your Consent Form was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Consent Form has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Consent Form</title>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">Consent Form Submission</h1>
                </div>

                <div class="template-box">
                    <a href="templates/endorsement/consent_form.docx" download="consent_form.docx" class="btn1-download-template"><img src="images/doc.png" alt="" class="doc-icon"><u>Consent_Form_Template.docx</u></a>
                    <a href="templates/endorsement/consent_form.docx" download="consent_form.docx" class="btn2-download-template btn-download">
                    ...
                    <div class="content">
                        <div class="document-area" id="documentArea">
                            <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                                ...
                                ?>
                                    <img id="documentImage" src="<?php echo htmlspecialchars($existing_document['uploaded_path']) . $cache_buster; ?>" alt="Consent Form Image">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="consentFormDocument" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden">
                ...
                editBtn.addEventListener('click', function() {
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This Consent Form has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This Consent Form has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
