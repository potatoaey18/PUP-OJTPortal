...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/nda/";
$relative_dir = "/PUP/student/uploads/nda/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["ndaDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["ndaDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["ndaDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["ndaDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if (!in_array($mime, $allowed_mimes)) {
        ...
        header("Location: nda.php");
        exit;
    }

    if ($_FILES["ndaDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["ndaDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["ndaDocument"]["tmp_name"], $target_file)) {
        error_log("File successfully moved to: " . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'nda'");
            ...
            if ($stmt->execute($params)) {
                ...
                $_SESSION['status'] = "NDA uploaded successfully. Awaiting review.";
        ...
        error_log("File move failed. Error: " . $_FILES["ndaDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["ndaDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'nda'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Non-Disclosure Agreement (NDA) has been approved.",
        'denied' => "Your NDA was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Non-Disclosure Agreement has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Student NDA</title>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">NDA Template</h1>
                </div>

                <div class="template-box">
                    <a href="templates/endorsement/nda_template.docx" download="nda_template.docx" class="btn1-download-template"><img src="images/doc.png" alt="" class="doc-icon"><u>NDA_Template.docx</u></a>
                    <a href="templates/endorsement/nda_template.docx" download="nda_template.docx" class="btn2-download-template btn-download">
                        <i class="fa fa-download"></i>
                    </a>
                </div>

                <div>
                    <h1 class="title">Non-Disclosure Agreement (NDA)</h1>
                </div>
                    ...
                    <div class="content">
                        <div class="document-area" id="documentArea">
                            <?php if ($existing_document): ?>
                                ...
                                ?>
                                    <img id="documentImage" src="<?php echo $existing_document['uploaded_path'] . $cache_buster; ?>" alt="NDA Image">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="ndaDocument" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden">
                ...
                editBtn.addEventListener('click', function() {
                    ...
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This NDA has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This NDA has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
            ...
            printBtn.addEventListener('click', function() {
                ...
                <?php if ($existing_document): ?>
                    ...
                    if (fileExt === 'pdf') {
                        ...
                        console.log('Opening non-PDF/image file');
