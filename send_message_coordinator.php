...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM conversations_coordinator_student WHERE coordinator_id = ? AND student_id = ?");
    $stmt->execute([$coordinator_id, $student_id]);
    $conversation = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$conversation) {
        // Create a new conversation if it doesn't exist
        $stmt = $conn->prepare("INSERT INTO conversations_coordinator_student (coordinator_id, student_id, created_at) VALUES (?, ?, NOW())");
        $stmt->execute([$coordinator_id, $student_id]);
    ...
    // First, get the student's email from the students_data table using student_id
    $stmt = $conn->prepare("SELECT stud_email FROM students_data WHERE id = ?");
    $stmt->execute([$student_id]);
    $student = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$student || empty($student['stud_email'])) {
        throw new Exception("Student email not found in database");
    }
    
    $student_email = $student['stud_email'];
    
    // Get the student's user_id from the users table using their email
    $stmt = $conn->prepare("SELECT user_id FROM users_coordinator_student WHERE email = ? AND user_type = 'student'");
    $stmt->execute([$student_email]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$user) {
        throw new Exception("Student not found in users table with email: " . $student_email);
    }
    
    $user_id = $user['user_id'];
    
    // Insert message using the user_id from the users table
    $stmt = $conn->prepare("INSERT INTO messages_coordinator_student (conversation_id, sender_id, sender_type, content, created_at) VALUES (?, ?, 'student', ?, NOW())");
    ...
    $stmt = $conn->prepare(
        ...
        LEFT JOIN users_coordinator_student u ON m.sender_id = u.user_id
        WHERE m.id = ?"
    );
    ...
    try {
        $stmt = $conn->prepare("UPDATE conversations_coordinator_student SET updated_at = NOW() WHERE id = ?");
    ...
    if (defined('PUSHER_APP_KEY') && !empty(PUSHER_APP_KEY)) {
        try {
            ...
            // Also trigger to student's channel for sync across devices
            $pusher->trigger(
                'private-coordinators-' . $coordinator_id,
                'new_message',
                [
