...
try {
    ...
    $stmt = $conn->prepare("SELECT user_id FROM users_coordinator_hte WHERE email = ? AND user_type = 'supervisor'");
    ...
    $stmt = $conn->prepare("INSERT INTO messages_coordinator_supervisor (conversation_id, sender_id, sender_type, content, created_at) VALUES (?, ?, 'supervisor', ?, NOW())");
    ...
    $stmt = $conn->prepare(
        ...
        LEFT JOIN users_coordinator_hte u ON m.sender_id = u.user_id
        WHERE m.id = ?"
    );
    ...
    try {
        $stmt = $conn->prepare("UPDATE conversations_coordinator_hte SET updated_at = NOW() WHERE id = ?");
