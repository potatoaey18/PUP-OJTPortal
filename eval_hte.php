...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/eval_hte/";
$relative_dir = "/PUP/student/uploads/eval_hte/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["eval_hteDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["eval_hteDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["eval_hteDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["eval_hteDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if (!in_array($mime, $allowed_mimes)) {
        ...
        header("Location: eval_hte.php");
        exit;
    }

    if ($_FILES["eval_hteDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["eval_hteDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["eval_hteDocument"]["tmp_name"], $target_file)) {
        error_log("File successfully moved to: " . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'eval_hte'");
        ...
        error_log("File move failed. Error: " . $_FILES["eval_hteDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["eval_hteDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'eval_hte'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your HTE Evaluation has been approved.",
        'denied' => "Your HTE Evaluation was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your HTE Evaluation has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: HTE Evaluation</title>
                ...
                <div>
                    ...
                    <div class="page-header">
                        <div class="page-title">
                            <h1 style="font-size: 16px;"><b>Evaluate Host Training Establishment</b></h1>
                        </div>
                    </div>
                    ...
                    <div>
                        <h1 class="title">Evaluate HTE</h1>
                    </div>
                    <div class="template-box">
                        <a href="templates/evaluations/HTE_Evaluation_Template.docx" download="HTE_Evaluation_Template.docx" class="btn1-download-template" style="margin-top: 105px;"><img src="images/doc.png" alt="" class="doc-icon"><u>HTE_Evaluation_Template.docx</u></a>
                        <a href="templates/evaluations/HTE_Evaluation_Template.docx" download="HTE_Evaluation_Template.docx" class="btn2-download-template btn-download" style="margin-top: 105px;">
                        ...
                        <div class="content">
                            ...
                            <div class="action-buttons">
                                <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                    <input type="file" id="fileInput" name="eval_hteDocument" accept=".pdf,.doc,.docx" class="hidden">
