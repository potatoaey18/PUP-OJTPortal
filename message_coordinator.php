        ...
        <div class="profile-container">
            <div class="messaging-container">
                ...
                <div class="contacts-panel">
                    ...
                    $student_id = $_SESSION['auth_user']['student_id'] ?? 0;
                    $student_name = 'Student';
                    $profile_picture = 'default-avatar.png'; // Default avatar if none is set
                    
                    if ($student_id) {
                        $stmt = $conn->prepare("SELECT first_name, middle_name, last_name, profile_picture FROM students_data WHERE id = :id");
                        $stmt->execute([':id' => $student_id]);
                        $student = $stmt->fetch(PDO::FETCH_ASSOC);
                        if ($student) {
                            $student_name = htmlspecialchars($student['first_name'] . ' ' . ($student['middle_name'] ? $student['middle_name'] . ' ' : '') . $student['last_name']);
                    ...
                    <div class="contacts-list">
                        ...
                        try {
                            ...
                            foreach ($coordinators as &$coordinator) {
                                $stmt = $conn->prepare("
                                    SELECT c.id 
                                    FROM conversations_coordinator_student c
                                    JOIN conversation_participants cp1 ON c.id = cp1.conversation_id
                                    JOIN conversation_participants cp2 ON c.id = cp2.conversation_id
                                    WHERE cp1.user_id = ? AND cp2.user_id = ?
                                    LIMIT 1
                                ");
                                $stmt->execute([$student_id, $coordinator['id']]);
                                $conversation = $stmt->fetch();
                                
                                if ($conversation) {
                                    $stmt = $conn->prepare("
                                        SELECT m.content, m.created_at 
                                        FROM messages_coordinator_student m 
    ...
    if (typeof Toastify !== 'undefined') {
        ...
        function initializePusher() {
            ...
            try {
                ...
                const studentId = '<?php echo isset($_SESSION['auth_user']['student_id']) ? $_SESSION['auth_user']['student_id'] : '0'; ?>';
                
                if (!appKey || !cluster || !studentId || studentId === '0') {
                    console.error('Pusher initialization failed - missing credentials', { appKey, cluster, studentId });
                ...
                const channel = pusher.subscribe('private-student-' + studentId);
                
                channel.bind('new_message', function(data) {
                    console.log('New message received:', data);
                    if (data.sender_type === 'coordinator') {
                        const messageId = data.id || 'msg_' + Date.now();
                        const isMe = data.sender_id === '<?php echo isset($_SESSION['auth_user']['student_id']) ? $_SESSION['auth_user']['student_id'] : '0'; ?>';
                ...
                channel.bind('pusher:subscription_succeeded', function() {
                    console.log('Successfully subscribed to channel: private-student-' + studentId);
                ...
                channel.bind('pusher:subscription_error', function(status, response) {
                    console.error('Pusher subscription error:', {
                        ...
                        channel: 'private-student-' + studentId
                    });
                    showToast('Error', `Failed to establish real-time messaging connection (Status: ${status}).`, 'error');
                });
                
                console.log('Pusher initialized for channel: private-student-' + studentId);
                return pusher;
            } catch (error) {
