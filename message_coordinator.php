...
<head>
    ...
    <script>
         ...
         // Global flag to control toast visibility
    const SHOW_TOASTS = false; // Set to false to disable toasts UI (messages will still log to console)
    
    // Function to show toast notifications
    function showToast(title, message, type = 'info') {
        // If first argument is an object (Toastify options)
        if (typeof title === 'object' && title.text) {
            if (!SHOW_TOASTS) {
                console.log(`[${title.type || 'info'}] ${title.text}`);
                return;
            }
            return Toastify(title).showToast();
        }
        
        // If called with separate title and message
        if (!SHOW_TOASTS) {
...
<body>
        ...
        <div class="profile-container">
            <div class="messaging-container">
                ...
                <div class="contacts-panel">
                    <?php
                    // Fetch supervisor's details from the database
                    $supervisor_id = $_SESSION['auth_user']['supervisor_id'] ?? 0;
                    $supervisor_name = 'Supervisor';
                    $profile_picture = 'default-avatar.png'; // Default avatar if none is set
                    
                    if ($supervisor_id) {
                        $stmt = $conn->prepare("SELECT first_name, middle_name, last_name, supervisor_profile_picture FROM supervisor WHERE id = :id");
                        $stmt->execute([':id' => $supervisor_id]);
                        $supervisor = $stmt->fetch(PDO::FETCH_ASSOC);
                        if ($supervisor) {
                            $supervisor_name = htmlspecialchars($supervisor['first_name'] . ' ' . ($supervisor['middle_name'] ? $supervisor['middle_name'] . ' ' : '') . $supervisor['last_name']);
                            if (!empty($supervisor['supervisor_profile_picture'])) {
                                $profile_picture = $supervisor['supervisor_profile_picture'];
                            }
                        }
                    }
                    ?>
                    ...
                    <div class="contacts-list">
                        ...
                        $supervisor_id = $_SESSION['auth_user']['supervisor_id'] ?? 0;
                        try {
                            $stmt = $conn->prepare("SELECT * FROM coordinators_account ORDER BY first_name, last_name");
                            $stmt->execute();
                            ...
                            foreach ($coordinators as &$coordinator) {
                                $stmt = $conn->prepare("
                                    SELECT c.id 
                                    FROM conversations_coordinator_hte c
                                    JOIN conversation_participants cp1 ON c.id = cp1.conversation_id
                                    JOIN conversation_participants cp2 ON c.id = cp2.conversation_id
                                    WHERE cp1.user_id = ? AND cp2.user_id = ?
                                    LIMIT 1
                                ");
                                $stmt->execute([$supervisor_id, $coordinator['id']]);
                                ...
                                if ($conversation) {
                                    ...
                                    $coordinator['last_message'] = $lastMessage ? $lastMessage['content'] : '';
                                    $coordinator['last_message_time'] = $lastMessage ? $lastMessage['created_at'] : null;
                                } else {
                                    $coordinator['last_message'] = '';
                                    $coordinator['last_message_time'] = null;
                                }
                            }
                            
                            // Sort the coordinators array by last message time
                            usort($coordinators, function($a, $b) {
                        ...
                        if (count($coordinators) > 0) {
                            foreach ($coordinators as $coordinator) {
                                ...
                                $fullName = htmlspecialchars($coordinator['first_name'] . ' ' . $coordinator['last_name']);
                                $status = $coordinator['online_offlineStatus'] === 'Online' ? 'online' : 'offline';
                                $lastMessage = '';
                                if (!empty($coordinator['last_message'])) {
                                    $lastMessage = htmlspecialchars($coordinator['last_message']);
                                    if (strlen($lastMessage) > 30) {
                                        $lastMessage = substr($lastMessage, 0, 27) . '...';
                                    }
                                }
                                
                                echo "<div class='contact-item' data-coordinator-id='{$coordinator['id']}' data-coordinator-course='".htmlspecialchars($coordinator['stud_course'] ?? 'No course specified')."'>";
                                ...
                                echo "<span class='preview-text' title='".htmlspecialchars($coordinator['last_message'] ?? '')."'>{$lastMessage}</span>";
                                echo "</div></div></div>";
                            }
                        } else {
                            echo "<div class='text-center p-4'>No coordinators found</div>";
                        }
                        ?>
                    </div>
                </div>
                ...
                <div class="chat-panel">
                    ...
                    <div id="chatContent" style="display: none;">
                        <div class="chat-header">
                            ...
                            <div class="chat-info">
                                <h5 id="chatAdminName">John Doe</h5>
                                <p id="chatAdminInfo">BS Computer Engineering - 3A</p>
                            </div>
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; height: calc(100% - 70px);">
                            ...
                            <div class="message-input-container">
                                ...
                                <div class="message-input-wrapper">
                                    <input type="hidden" id="currentCoordinatorId">
    ...
    $(document).ready(function() {
        let currentCoordinatorId = null; // Global variable for current student ID

        // Function to save messages to localStorage
        function saveMessages(coordinatorId, messages) {
            localStorage.setItem(`messages_${coordinatorId}`, JSON.stringify(messages));
        }

        // Function to load messages from localStorage
        function loadSavedMessages(coordinatorId) {
            const saved = localStorage.getItem(`messages_${coordinatorId}`);
        ...
        function sortContactsByLastMessage() {
            ...
            $contacts.sort((a, b) => {
                const aId = $(a).data('coordinator-id');
                const bId = $(b).data('coordinator-id');
        ...
        function updateContactPreview(coordinatorId, message, isNewMessage = true, previewTime = null) {
            console.log('Updating contact preview for coordinator:', coordinatorId, 'Message:', message, 'isNewMessage:', isNewMessage, 'previewTime:', previewTime);
            
            const contactItem = document.querySelector(`.contact-item[data-coordinator-id="${coordinatorId}"]`);
            if (!contactItem) {
                console.error('Contact item not found for coordinator ID:', coordinatorId);
                return null;
            }
            ...
            previews[coordinatorId] = previewData;
        ...
        function renderMessages(messages) {
            ...
            if (messages.length > 0 && currentCoordinatorId) {
                const lastMessage = messages[messages.length - 1];
                updateContactPreview(currentCoordinatorId, lastMessage.text, false);
            }
            
            setTimeout(scrollToBottom, 50);
        }

        // Function to load messages for a student
        function loadAdminMessages(coordinatorId) {
            if (!coordinatorId) return;
            
            console.log('Loading messages for coordinator ID:', coordinatorId);
            currentCoordinatorId = coordinatorId;
            $('#currentCoordinatorId').val(coordinatorId);
            
            $('.contact-item').removeClass('active');
            $(`.contact-item[data-coordinator-id="${coordinatorId}"]`).addClass('active');
            
            const contactItem = $(`.contact-item[data-coordinator-id="${coordinatorId}"]`);
            const coordinatorName = contactItem.find('.contact-name').text().trim().split(' ')[0] + ' ' + contactItem.find('.contact-name').text().trim().split(' ')[1];
            $('#chatAdminName').text(coordinatorName || 'Coordinator');
            const initials = coordinatorName.split(' ').map(n => n[0]).join('').toUpperCase();
            $('#chatAvatar').text(initials);
            
            // Get the course from the data attribute and update the chat info
            const coordinatorCourse = contactItem.data('coordinator-course') || 'No course specified';
            $('#chatAdminInfo').text(coordinatorCourse);
            ...
            const savedMessages = loadSavedMessages(coordinatorId);
            ...
            $.ajax({
                url: 'get_message_coordinator.php',
                type: 'GET',
                data: { coordinator_id: coordinatorId },
                dataType: 'json',
                success: function(response) {
                    console.log('Messages loaded:', response);
                    if (response.status === 'success' && response.data && Array.isArray(response.data.messages)) {
                        const formattedMessages = response.data.messages.map(msg => ({
                            id: msg.id,
                            conversation_id: response.data.conversation_id,
                            sender: msg.sender_type === 'supervisor' ? 'me' : 'them',
                            text: msg.content,
                            time: formatTime(msg.created_at),
                            is_read: msg.is_read,
                            created_at: msg.created_at,
                            sender_name: msg.sender_name || 'Unknown Sender'
                        }));
                        
                        saveMessages(coordinatorId, formattedMessages);
                        if (!savedMessages || savedMessages.length !== formattedMessages.length) {
                            renderMessages(formattedMessages);
                        }
                        
                        markMessagesAsRead(coordinatorId);
                    } else {
        ...
        function markMessagesAsRead(coordinatorId) {
            if (!coordinatorId) return;
            
            $('.message[data-coordinator-id="' + coordinatorId + '"]').removeClass('unread').addClass('read');
            updateUnreadCount(coordinatorId, false);
            
            const unreadMessages = [];
            $('.message[data-coordinator-id="' + coordinatorId + '"].unread').each(function() {
        ...
        function updateUnreadCount(coordinatorId, increment = false) {
            const contactItem = $(`.contact-item[data-coordinator-id="${coordinatorId}"]`);
        ...
        function sendMessage() {
            ...
            console.log('Message to send:', message, ', to coordinator:', currentCoordinatorId);
            
            if (!message || !currentCoordinatorId) {
                console.log('Message or coordinator ID is missing');
                return;
            }
            ...
            const existingMessages = loadSavedMessages(currentCoordinatorId);
            existingMessages.push(newMessage);
            saveMessages(currentCoordinatorId, existingMessages);
            renderMessages(existingMessages);
            updateContactPreview(currentCoordinatorId, message, true);
            messageInput.val('').focus();
            scrollToBottom();
            
            $.ajax({
                url: 'send_message_coordinator.php',
                type: 'POST',
                data: {
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success' && response.data) {
                        const messages = loadSavedMessages(currentCoordinatorId);
                        ...
                        if (messageIndex !== -1) {
                            ...
                            saveMessages(currentCoordinatorId, messages);
        ...
        function loadContactPreviews() {
            ...
            $('.contact-item').each(function() {
                const coordinatorId = $(this).data('coordinator-id');
                const previewData = previews[coordinatorId];
                ...
                if (previewData) {
                    ...
                    previews[coordinatorId] = newPreview;
        ...
        function selectConversation(coordinatorId, coordinatorName) {
            currentCoordinatorId = coordinatorId;
            $('#currentCoordinatorId').val(coordinatorId);
            
            $('.contact-item').removeClass('active');
            $(`[data-coordinator-id="${coordinatorId}"]`).addClass('active');
            
            const contactItem = $(`[data-coordinator-id="${coordinatorId}"]`);
            const status = contactItem.find('.contact-status').text().trim();
            const program = 'BS Computer Engineering'; // Default program
            
            $('#chatAvatar').text(coordinatorName.split(' ').map(n => n[0]).join('').toUpperCase());
            $('#chatAdminName').text(coordinatorName);
            $('#chatAdminInfo').text(`${program} • ${status}`);
            ...
            loadAdminMessages(coordinatorId);
            updateFileCounts();
        ...
        function initializePusher() {
            ...
            try {
                ...
                const supervisorId = '<?php echo isset($_SESSION['auth_user']['supervisor_id']) ? $_SESSION['auth_user']['supervisor_id'] : '0'; ?>';
                
                if (!appKey || !cluster || !supervisorId || supervisorId === '0') {
                    console.error('Pusher initialization failed - missing credentials', { appKey, cluster, supervisorId });
                ...
                const channel = pusher.subscribe('private-supervisor-' + supervisorId);
                
                channel.bind('new_message', function(data) {
                    console.log('New message received:', data);
                    if (data.sender_type === 'coordinator') {
                        const messageId = data.id || 'msg_' + Date.now();
                        const isMe = data.sender_id === '<?php echo isset($_SESSION['auth_user']['supervisor_id']) ? $_SESSION['auth_user']['supervisor_id'] : '0'; ?>';
                        const previewTime = data.created_at ? formatTime(data.created_at) : new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                        const newMessage = {
                            id: messageId,
                            sender: isMe ? 'me' : 'them',
                            text: data.content,
                        ...
                        const senderId = data.sender_id || data.coordinator_id;
                        if (!currentCoordinatorId && senderId) {
                            currentCoordinatorId = senderId;
                            $('#currentCoordinatorId').val(senderId);
                        }
                        
                        const existingMessages = loadSavedMessages(senderId || currentCoordinatorId);
                        const messageExists = existingMessages.some(msg => msg.id === messageId || (msg.text === data.content && msg.sender === (isMe ? 'me' : 'them')));
                        
                        if (!messageExists) {
                            existingMessages.push(newMessage);
                            saveMessages(senderId || currentCoordinatorId, existingMessages);
                            
                            // Update preview time regardless of the current chat
                            updateContactPreview(senderId || currentCoordinatorId, data.content, true, previewTime);
                            
                            if (senderId !== currentCoordinatorId) {
                                showToast('New Message', `New message from coordinator ${senderId}`, 'info');
                                updateUnreadCount(senderId, true);
                            }
                            
                            if (!currentCoordinatorId || senderId === currentCoordinatorId) {
                                renderMessages(existingMessages);
                                if (senderId === currentCoordinatorId) {
                ...
                channel.bind('pusher:subscription_succeeded', function() {
                    console.log('Successfully subscribed to channel: private-supervisor-' + supervisorId);
                ...
                channel.bind('pusher:subscription_error', function(status, response) {
                    console.error('Pusher subscription error:', {
                        ...
                        channel: 'private-supervisor-' + supervisorId
                    });
                    showToast('Error', `Failed to establish real-time messaging connection (Status: ${status}).`, 'error');
                });
                
                console.log('Pusher initialized for channel: private-supervisor-' + supervisorId);
                return pusher;
            } catch (error) {
        ...
        $(document).on('click', '.contact-item', function() {
            const coordinatorId = $(this).data('coordinator-id');
            const coordinatorName = $(this).find('.contact-name').text().trim();
            if (coordinatorId) {
                selectConversation(coordinatorId, coordinatorName);
            }
        });
