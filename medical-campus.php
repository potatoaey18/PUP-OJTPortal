...
$document_paths = [];
try {
    $query = "SELECT document_type, uploaded_path, status, remarks FROM endorsement_documents WHERE student_id = :student_id AND medical_at_campus = 'YES'";
    $stmt = $conn->prepare($query);
    $stmt->bindValue(':student_id', $student_id, PDO::PARAM_INT);
    $stmt->execute();
    ...
    foreach ($results as $row) {
        ...
        $document_paths[$row['document_type']] = [
            'path' => $row['uploaded_path'],
            'status' => $row['status'],
            'remarks' => $row['remarks']
        ];
    }
...
<body>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 5rem; margin-left: 15rem; padding: 2rem;">
            ...
            <div class="box">
                <form action="" method="POST" enctype="multipart/form-data" id="medicalForm">
                    <div class="document-row" onclick="selectDocument('cor')">
                        ...
                        <button type="button" class="edit-upload-btn" onclick="document.getElementById('cor_file').click();" <?php echo in_array('cor_file', $existing_uploads) && $document_paths['cor_file']['status'] !== 'denied' ? 'disabled' : ''; ?>>Upload</button>
                        <?php if (isset($document_paths['cor_file']) && $document_paths['cor_file']['status'] === 'denied' && !empty($document_paths['cor_file']['remarks'])): ?>
                            <div class="notifs-container">
                                <div class="remarks-notification" id="remarksNotification_cor" style="display: block;">
                                    <span class="close-btn" onclick="this.parentElement.style.display='none'">×</span>
                                    <strong>Remarks:</strong> <?php echo htmlspecialchars($document_paths['cor_file']['remarks']); ?>
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="document-row" onclick="selectDocument('xray')">
                        ...
                        <button type="button" class="edit-upload-btn" onclick="document.getElementById('xray_file').click();" <?php echo in_array('xray_file', $existing_uploads) && $document_paths['xray_file']['status'] !== 'denied' ? 'disabled' : ''; ?>>Upload</button>
                        <?php if (isset($document_paths['xray_file']) && $document_paths['xray_file']['status'] === 'denied' && !empty($document_paths['xray_file']['remarks'])): ?>
                            <div class="notifs-container">
                                <div class="remarks-notification" id="remarksNotification_xray" style="display: block;">
                                    <span class="close-btn" onclick="this.parentElement.style.display='none'">×</span>
                                    <strong>Remarks:</strong> <?php echo htmlspecialchars($document_paths['xray_file']['remarks']); ?>
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="document-row" onclick="selectDocument('declaration')">
                        ...
                        <div class="document-name"><a href="templates/endorsement/Declaration_of_Medical_Information_and_Data_Subject_Consent_Form.docx" download="Declaration_of_Medical_Information_and_Data_Subject_Consent_Form.docx"><u>Declaration of Medical Information and Data Subject Consent Form</u></a></div>
                        <input type="file" name="declaration_file" id="declaration_file" accept=".pdf" class="hidden">
                        <button type="button" class="edit-upload-btn" onclick="document.getElementById('declaration_file').click();" <?php echo in_array('declaration_file', $existing_uploads) && $document_paths['declaration_file']['status'] !== 'denied' ? 'disabled' : ''; ?>>Upload</button>
                        <?php if (isset($document_paths['declaration_file']) && $document_paths['declaration_file']['status'] === 'denied' && !empty($document_paths['declaration_file']['remarks'])): ?>
                            <div class="notifs-container">
                                <div class="remarks-notification" id="remarksNotification_declaration" style="display: block;">
                                    <span class="close-btn" onclick="this.parentElement.style.display='none'">×</span>
                                    <strong>Remarks:</strong> <?php echo htmlspecialchars($document_paths['declaration_file']['remarks']); ?>
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="document-row" onclick="selectDocument('health')">
                        ...
                        <div class="document-name"><a href="templates/endorsement/Health_Information_Form_for_Students.docx" download="Health_Information_Form_for_Students.docx"><u>Health Information Form for Students</u></a></div>
                        <input type="file" name="health_info_file" id="health_info_file" accept=".pdf" class="hidden">
                        <button type="button" class="edit-upload-btn" onclick="document.getElementById('health_info_file').click();" <?php echo in_array('health_info_file', $existing_uploads) && $document_paths['health_info_file']['status'] !== 'denied' ? 'disabled' : ''; ?>>Upload</button>
                        <?php if (isset($document_paths['health_info_file']) && $document_paths['health_info_file']['status'] === 'denied' && !empty($document_paths['health_info_file']['remarks'])): ?>
                            <div class="notifs-container">
                                <div class="remarks-notification" id="remarksNotification_health" style="display: block;">
                                    <span class="close-btn" onclick="this.parentElement.style.display='none'">×</span>
                                    <strong>Remarks:</strong> <?php echo htmlspecialchars($document_paths['health_info_file']['remarks']); ?>
                                </div>
                            </div>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        let documentPaths = <?php echo json_encode(array_map(function($doc) { return $doc['path']; }, $document_paths)); ?>;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            Object.keys(fileInputs).forEach(inputId => {
                ...
                if (existingUploads.includes(inputId)) {
                    if (documentPaths[inputId]?.status !== 'denied') {
            ...
            if (selectedDocument) {
                ...
                if (fileInput.files.length > 0) {
                    ...
                    if (typeof showErrorAlert === 'function') {
                        showErrorAlert("Notice", "No document available to view or no document selected");
                    } else {
                        console.warn("showErrorAlert is not defined. Falling back to native alert.");
                        alert("Notice: No document available to view or no document selected");
                    }
                }
            }
        });
        ...
        document.getElementById('printBtn').addEventListener('click', function(e) {
            ...
            if (selectedDocument) {
                ...
                if (filePath) {
                    ...
                    if (typeof showErrorAlert === 'function') {
                        showErrorAlert("Notice", "No document available to print or no document selected");
                    } else {
                        console.warn("showErrorAlert is not defined. Falling back to native alert.");
                        alert("Notice: No document available to print or no document selected");
                    }
                }
            }
        });
