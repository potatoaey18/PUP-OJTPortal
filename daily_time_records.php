                    ...
                    <tbody>
                        ...
                        if (!empty($students)) {
                            foreach ($students as $student) {
                                ...
                                $deptStmt = $conn->prepare("SELECT department FROM intern_deployments WHERE intern_id = ? ORDER BY id DESC LIMIT 1");
                                $deptStmt->execute([$student['student_ID']]);
                                $deployment = $deptStmt->fetch(PDO::FETCH_ASSOC);
                                $assignedDept = $deployment ? $deployment['department'] : '-';
                              
                                $raw_dept = trim($assignedDept);
                                ...
                                $displayDept = htmlspecialchars($displayDept);
                                $full_name = htmlspecialchars($student['last_name'] . ', ' . $student['first_name'] . ' ' . $student['middle_name']);
                                $status = htmlspecialchars($student['ojt_status'] ? $student['ojt_status'] : '-');
                                echo "<td>$displayDept</td>";
                                echo "<td>" . htmlspecialchars($student['student_ID']) . "</td>";
                                echo "<td>" . htmlspecialchars($student['stud_section']) . "</td>";
                                echo "<td>$full_name</td>";
                                echo "<td>$status</td>";
                                echo "<td><a href='view_dtr1.php?student_id=" . urlencode($student['student_ID']) . "' class='view-profile-btn'>View DTR</a></td>";
                                echo "<td><a href='view_student_profile.php?intern_id=" . urlencode($student['student_ID']) . "' class='view-profile-btn'>View Profile</a></td>";
                                echo "</tr>";
                            }
                        } else {
                            echo "<tr><td colspan='8'>No students found for your company.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
