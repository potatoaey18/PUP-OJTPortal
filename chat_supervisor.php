...
try {
    ...
    // Get supervisor's details from database
    $stmt = $conn->prepare("SELECT company_name, supervisor_profile_picture FROM supervisor WHERE id = ?");
    if (!$stmt) {
    ...
    // Get company name and profile picture
    $companyName = $supervisor['company_name'] ?? 'No Company';
    $currentImagePath = $supervisor['supervisor_profile_picture'] ?? 'images/default_profile.png';

    // Fetch students assigned to this supervisor's company
    $stmt = $conn->prepare("SELECT students_data.* 
                           FROM students_data 
                           INNER JOIN deployed_students ON students_data.id = deployed_students.student_id 
