...
<body>
    ...
    <div class="content-wrap">
        <div class="page-container">
            ...
            <div class="page-title">Weekly Accomplishment Report</div>

            <?php

if (!$student_id) {
    echo '<div style="color:red; font-weight:bold;">No student_id provided in URL.</div>';
}
?>
<div class="intern-info">
    ...
    echo '<!-- DEBUG: student_id = ' . htmlspecialchars($student_id) . ' -->';
    if ($student_id) {
 
    echo '<!-- DEBUG: Running student fetch for student_id = ' . htmlspecialchars($student_id) . ' -->';
        
                    $stmt = $conn->prepare("SELECT first_name, middle_name, last_name, ojt_status FROM students_data WHERE student_ID = ? LIMIT 1");
                    $stmt->execute([$student_id]);
                    if ($row = $stmt->fetch()) {
   
    echo '<!-- DEBUG: student data found for student_id = ' . htmlspecialchars($student_id) . ' -->';
                        $intern_name = htmlspecialchars($row['last_name'] . ', ' . $row['first_name'] . ' ' . $row['middle_name']);
                        $intern_status = htmlspecialchars($row['ojt_status']);
                        $ojt_adviser = 'N/A'; 
                    }
                 
                    $dept_stmt = $conn->prepare("SELECT department FROM intern_deployments WHERE intern_id = ? ORDER BY deployment_date DESC LIMIT 1");
                ...
                <p><strong>OJT Adviser/s</strong><span class="colon">:</span> <?= $ojt_adviser ?></p>
            ...
            <div class="table-container">
                <table class="table">
                    <thead>
                        <tr>
                            <th colspan="5">
                                <div class="table-header">Weekly Accomplishment</div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
    ...
    <td>Week #</td>
    <td>Weekly Report</td>
    <td>Name/Number of Co-workers</td>
...
for ($week = 1; $week <= 3; $week++) {
    ...
    echo "<td>Sample accomplishment for week $week</td>";
    echo "<td>John Doe, Jane Smith</td>";
    echo "<td>40</td>";
    echo "</tr>";
}
?>
