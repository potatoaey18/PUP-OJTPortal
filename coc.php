...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/coc/";
$relative_dir = "/PUP/student/uploads/coc/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["cocDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["cocDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["cocDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["cocDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if ($_FILES["cocDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["cocDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["cocDocument"]["tmp_name"], $target_file)) {
        ...
        error_log("File move failed. Error: " . $_FILES["cocDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["cocDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'coc'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Certificate of Completion has been approved.",
        'denied' => "Your Certificate of Completion was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Certificate of Completion has been submitted and is under review."
    ];
...
<head>
    ...
    <style>
        ...
        .notifs-container {
            padding-top: 10px; /* Reduced padding for better layout */
        }
    </style>
</head>
<body>
            ...
            <div>
                <div class="page-header">
                    <div class="page-title">
                        <h1 style="font-size: 16px;"><b>Certificate of Completion</b></h1>
                    </div>
                </div><br><br>
                <div>
                    ...
                    <div class="content">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="cocDocument" accept=".pdf,.doc,.docx" class="hidden">
