...
function logError($message) {
    error_log('[Endorsement Upload] ' . $message);
}
...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/endorsement/";
$relative_dir = "/PUP/student/uploads/endorsement/";

// Fetch existing document before handling form submission
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           FROM endorsement_documents 
                           WHERE student_id = ? AND document_type = 'endorsement'");
...
if (isset($_POST['submit'])) {
    ...
    if (!isset($_FILES["endorsementDocument"]) || $_FILES["endorsementDocument"]["error"] !== UPLOAD_ERR_OK) {
        logError("File upload error: " . ($_FILES["endorsementDocument"]["error"] ?? 'No file uploaded'));
        echo json_encode([
    ...
    $original_name = basename($_FILES["endorsementDocument"]["name"]);
    ...
    logError("Temporary file path: " . $_FILES["endorsementDocument"]["tmp_name"]);
    logError("Upload error code: " . $_FILES["endorsementDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["endorsementDocument"]["tmp_name"]);
    finfo_close($finfo);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if (!in_array($mime, $allowed_mimes)) {
        ...
        echo json_encode([
            'status' => 'Invalid file type. Only PDF, DOC, and DOCX are allowed.',
            'status_code' => 'error',
            'alert' => 'Error'
        ]);
        exit;
    ...
    if ($_FILES["endorsementDocument"]["size"] > 10000000) {
        logError("File size exceeds limit: " . $_FILES["endorsementDocument"]["size"]);
        echo json_encode([
    ...
    if (move_uploaded_file($_FILES["endorsementDocument"]["tmp_name"], $target_file)) {
        ...
        try {
            ...
            if ($stmt->execute($params)) {
                ...
                echo json_encode([
                    'status' => 'Endorsement uploaded successfully. Awaiting review.',
                    'status_code' => 'success',
                    'alert' => 'Success'
                ]);
            } else {
        ...
        logError("File move failed. Error: " . $_FILES["endorsementDocument"]["error"]);
        echo json_encode([
            'status' => 'Upload failed. Error: ' . $_FILES["endorsementDocument"]["error"],
            'status_code' => 'error',
            'alert' => 'Error'
        ]);
    }
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Endorsement has been approved.",
        'denied' => "Your Endorsement was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Endorsement has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Endorsement</title>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">Endorsement Submission</h1>
                </div>

                <div class="template-box">
                    <a href="templates/endorsement/endorsement.docx" download="endorsement.docx" class="btn1-download-template"><img src="images/doc.png" alt="" class="doc-icon"><u>Endorsement_Template.docx</u></a>
                    <a href="templates/endorsement/endorsement.docx" download="endorsement.docx" class="btn2-download-template btn-download">
                    ...
                    <div class="content">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="endorsementDocument" accept=".pdf,.doc,.docx" class="hidden">
                ...
                editBtn.addEventListener('click', function() {
                    ...
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This Endorsement has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This Endorsement has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
