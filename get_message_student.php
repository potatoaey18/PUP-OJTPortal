...
if (!isset($_SESSION['auth_user']['coordinators_id'])) {
    error_log('Unauthorized in get_message_student.php: Session=' . print_r($_SESSION, true));
    sendJsonResponse([
        'status' => 'error',
        'message' => 'Unauthorized: Coordinator not authenticated',
...
try {
    ...
    // Verify the student exists and is assigned to this coordinator
    $query = "SELECT id FROM students_data WHERE id = :student_id";
    error_log("Executing query: $query with student_id: $studentId");
    ...
    try {
        ...
        error_log("Student $studentId found");
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM conversations_coordinator_student WHERE student_id = :student_id AND coordinator_id = :coordinator_id");
        $stmt->bindParam(':student_id', $studentId, PDO::PARAM_INT);
        ...
        if (!$conversation) {
            ...
            $stmt = $conn->prepare("INSERT INTO conversations_coordinator_student (student_id, coordinator_id) VALUES (:student_id, :coordinator_id)");
            $stmt->bindParam(':student_id', $studentId, PDO::PARAM_INT);
            $stmt->bindParam(':coordinator_id', $coordinatorId, PDO::PARAM_INT);
        ...
        $query = "SELECT 
                    ...
                    CASE 
                        WHEN m.sender_type = 'student' AND sd.id IS NOT NULL THEN CONCAT(sd.first_name, ' ', sd.last_name)
                        WHEN m.sender_type = 'coordinator' AND co.id IS NOT NULL THEN CONCAT(co.first_name, ' ', co.last_name)
                        ELSE 'Unknown Sender'
                    END as sender_name,
                    sd.stud_email as student_email,
                    co.coordinators_email as coordinators_email
                FROM messages_coordinator_student m
                LEFT JOIN students_data sd ON sd.id = m.sender_id AND m.sender_type = 'student'
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $message = [
                ...
                'email' => $row['sender_type'] === 'student' ? ($row['student_email'] ?? null) : ($row['coordinators_email'] ?? null),
            ...
            // Track unread messages from student
            if ($row['sender_type'] === 'student' && !$row['is_read']) {
                $unreadMessageIds[] = (int)$row['id'];
            }
        }
        
        // Mark messages as read if they're from the student
        if (!empty($unreadMessageIds)) {
            $placeholders = rtrim(str_repeat('?,', count($unreadMessageIds)), ',');
            $updateQuery = "UPDATE messages_coordinator_student SET is_read = 1 WHERE id IN ($placeholders)";
