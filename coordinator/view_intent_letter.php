...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'intent_Letter'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
    ...
    error_log("Internship Agreement fetch error: " . $e->getMessage());
    $il = null;
}
?>
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                            <legend>Intent Letter</legend>
                        <div class="overview-cont">
                            <?php if ($il): ?>
                                <div class="moa-preview" id="ilPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($il['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $il['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $il['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                                        <p class="no-file-message">Preview not available for this file type. Use the "View" button to open it.</p>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                                <p class="no-file-message">No consent form document has been submitted.</p>
                            <?php endif; ?>
                        </div>
                    </fieldset>
                </div>
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($il): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($il['document_name']); ?></div>
                                </div>

                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($il['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $il ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $il['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $il && $il['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $il && $il['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
<script>
    ...
    function viewMoa() {
        <?php if ($il): ?>
            ...
            const ilPreview = document.getElementById('ilPreview');
            const overlay = document.getElementById('overlay');

            if (fileExt === 'pdf') {
                ilPreview.classList.toggle('enlarged');
                overlay.classList.toggle('active');
            } else {
                swal("Notice", "Preview not available for this file type", "info");
            }
        <?php else: ?>
            swal("Notice", "No intent letter document available to view", "info");
        <?php endif; ?>
    }

    function updateStatus(documentId, status, remarks = null) {
        if (documentId === null) {
            swal("Error", "No consent form document available to update.", "error");
            return;
        }
        swal({
            title: `Are you sure?`,
            text: `You are about to mark this Consent Form as ${status}.`,
            type: "warning",
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        remarksBtn.addEventListener('click', function() {
            const documentId = <?php echo $il['document_id'] ?? 'null'; ?>;
        ...
        overlay.addEventListener('click', function() {
            const ilPreview = document.getElementById('ilPreview');
            ilPreview.classList.remove('enlarged');
