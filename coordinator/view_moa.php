...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'moa'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
            ...
            <div class="content-holder">
                        ...
                        <div class="overview-cont">
                            <?php if ($moa): ?>
                                <div class="moa-preview" id="moaPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($moa['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $moa['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $moa['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                ...
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($moa): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($moa['document_name']); ?></div>
                                </div>
                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($moa['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $moa ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $moa['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $moa && $moa['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $moa && $moa['status'] !== 'denied' ? '' : 'disabled'; ?>>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            remarksBtn.addEventListener('click', function() {
                const documentId = <?php echo $moa['document_id'] ?? 'null'; ?>;
