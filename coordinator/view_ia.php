...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'internship_agreement'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
        ...
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        <legend>Internship Agreement</legend>
                        <div class="overview-cont">
                            <?php if ($ia): ?>
                                <div class="moa-preview" id="iaPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($ia['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $ia['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $ia['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                                        <p class="no-file-message">Preview not available for this file type. Use the "View" button to open it.</p>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                                <p class="no-file-message">No Internship Agreement document has been submitted.</p>
                            <?php endif; ?>
                        </div>
                    </fieldset>
                </div>
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($ia): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($ia['document_name']); ?></div>
                                </div>

                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($ia['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $ia ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $ia['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $ia && $ia['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $ia && $ia['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
<script>
    ...
    function viewMoa() {
        <?php if ($ia): ?>
            ...
            if (fileExt === 'pdf') {
                iaPreview.classList.toggle('enlarged');
    ...
    function updateStatus(documentId, status, remarks = null) {
        if (documentId === null) {
            swal("Error", "No internship agreement document available to update.", "error");
            return;
        }
        swal({
            title: `Are you sure?`,
            text: `You are about to mark this Internship Agreement as ${status}.`,
            type: "warning",
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        remarksBtn.addEventListener('click', function() {
            const documentId = <?php echo $ia['document_id'] ?? 'null'; ?>;
        ...
        overlay.addEventListener('click', function() {
            const iaPreview = document.getElementById('iaPreview');
            iaPreview.classList.remove('enlarged');
