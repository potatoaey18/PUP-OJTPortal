...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'nda'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
    $stmt->execute([$student_id]);
    $nda = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log("Non-Disclosure Agreement fetch error: " . $e->getMessage());
    $nda = null;
}
?>
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        ...
                        <div class="overview-cont">
                            <?php if ($nda): ?>
                                <div class="moa-preview" id="ndaPreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($nda['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $nda['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $nda['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                ...
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($nda): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($nda['document_name']); ?></div>
                                </div>

                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($nda['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $nda ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $nda['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $nda && $nda['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $nda && $nda['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
<script>
    ...
    function viewMoa() {
       <?php if ($nda): ?>
            ...
            if (fileExt === "pdf") {
                ndaPreview.classList.toggle('enlarged');
            ...
            <?php else: ?>
                swal("Notice", "No NDA document available for preview", "info");
            <?php endif; ?>
    }

    function updateStatus(documentId, status, remarks = null) {
        if (documentId === null) {
            swal("Error", "No Non-Disclosure Agreement (NDA) document available to update.", "error");
            return;
        }
        swal({
            title: `Are you sure?`,
            text: `You are about to mark this Non-Disclosure Agreement as ${status}.`,
            type: "warning",
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        remarksBtn.addEventListener('click', function() {
            const documentId = <?php echo $nda['document_id'] ?? 'null'; ?>;
        ...
        overlay.addEventListener('click', function () {
            ndaPreview = document.getElementById("ndaPreview");
            ndaPreview.classList.remove("enlarged");
