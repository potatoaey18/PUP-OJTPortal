            ...
            <div class="page-header">
              ...
              <div>
                ...
                <div>
                    ...
                    $stmt = $conn->prepare("SELECT * FROM supervisor");
                    $stmt->execute();
                    ...
                    ?>
                      <a href="javascript:void(0);" class="list-group-item list-group-item-action border-0" onclick="loadConversation('<?php echo $res['uniqueID']; ?>');">
                        ...
                        <div class="d-flex align-items-start">
                          <img src="<?php echo $res['supervisor_profile_picture']; ?>" class="rounded-circle mr-1" alt="<?php echo $res['first_name'] . ' ' . $res['middle_name'] . ' ' . $res['last_name']; ?>" width="40" height="40">
  ...
  $(function(){
    <?php foreach ($results as $res) { ?>
      ...
      function updateMessageCount_<?php echo $res['uniqueID']; ?>() {
        $.ajax({
          type: "POST",
          url: "load_supervisor_received_NewMessage.php",
...
<script>
  function loadConversation(user_uniqueId_receiver) {
    ...
    window.location.href = 'chat_supervisor.php?userUNIQUEid_receiver=' + user_uniqueId_receiver;
  }
  ...
  function loadSelectedUserConversation() {
    ...
    if (user_uniqueId_receiver) {
      ...
      $.ajax({
        url: 'chat_supervisor_messageLIVECHAT.php', // Replace with the actual URL to load conversation data
        method: 'POST',
        data: {
