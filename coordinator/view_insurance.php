...
<script>
    ...
    function viewMoa() {
        ...
        <?php else: ?>
            swal("Notice", "No insurance document available for preview", "info");
        <?php endif; ?>
    }

    function updateStatus(documentId, status, remarks = null) {
        if (documentId === null) {
            swal("Error", "No Insurance document available to update.", "error");
            return;
        }
        swal({
            title: `Are you sure?`,
            text: `You are about to mark this Insurance as ${status}.`,
            type: "warning",
