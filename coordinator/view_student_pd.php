<?php
    ...
    $student_id = $_GET['id'] ?? '';
    if (empty($student_id)) {
        header('Location: list_of_student_ie.php');
        exit;
    }
    ...
    try {
        $stmt = $conn->prepare("
            ...
            WHERE 
                student_id = ? AND document_type = 'photo_documentation'
            ORDER BY
                upload_date DESC
            LIMIT 1
        ");
            ...
            <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
                ...
                <div class="content-holder">
                    <div class="file-overview-holder">
                        <fieldset>
                            <legend>Photo Documentation</legend>
                            <div class="overview-cont">
                                <?php if ($pd): ?>
                                    <div class="moa-preview" id="iePreview">
                                        <?php
                                        $file_ext = strtolower(pathinfo($pd['uploaded_path'], PATHINFO_EXTENSION));
                                        if ($file_ext === 'pdf'): ?>
                                            <iframe src="<?php echo $pd['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                        <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                            <img src="<?php echo $pd['uploaded_path']; ?>" alt="MOA Image">
                                        <?php else: ?>
                                            <p class="no-file-message">Preview not available for this file type. Use the "View" button to open it.</p>
                                        <?php endif; ?>
                                    </div>
                                <?php else: ?>
                                    <p class="no-file-message">No Photo Documentation document has been submitted.</p>
                                <?php endif; ?>
                            </div>
                        </fieldset>
                    </div>
                    <div class="info-function-cont">
                            ...
                            <div class="moa-overview">
                                <?php if ($pd): ?>
                                    <div class="data-row">
                                        ...
                                        <div class="student-data">: <?php echo htmlspecialchars($pd['document_name']); ?></div>
                                    </div>
                                    <div class="data-row">
                                        <div class="label"><p><b>Status</b></p></div>
                                        <div class="student-data">: <?php echo htmlspecialchars(ucfirst($pd['status'])); ?></div>
                                    </div>
                                <?php else: ?>
                                    <div class="data-row">
                        ...
                        <div class="button-function">
                            <button class="btn-style" onclick="viewMoa()" <?php echo $pd ? '' : 'disabled'; ?>>
                                <i class="fa-regular fa-eye"></i> View
                            </button>
                            <button class="btn-style" onclick="updateStatus(<?php echo $pd['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $pd && $pd['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                                <i class="fa-solid fa-circle-check"></i> Approve
                            </button>
                            <button class="btn-style" onclick="toggleReject()" <?php echo $pd && $pd['status'] !== 'denied' ? '' : 'disabled'; ?>>
        ...
        <script>
            ...
            function viewMoa() {
                <?php if ($pd): ?>
                    ...
                    if (fileExt === 'pdf') {
                        iePreview.classList.toggle('enlarged');
            ...
            function updateStatus(documentId, status, remarks = null) {
                if (documentId === null) {
                    swal("Error", "No Photo Documentation available to update.", "error");
                    return;
                }
                swal({
                    title: `Are you sure?`,
                    text: `You are about to mark this Photo Dcoumentation as ${status}.`,
                    type: "warning",
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                remarksBtn.addEventListener('click', function() {
                    const documentId = <?php echo $pd['document_id'] ?? 'null'; ?>;
                ...
                overlay.addEventListener('click', function() {
                    const iePreview = document.getElementById('iePreview');
                    iePreview.classList.remove('enlarged');
