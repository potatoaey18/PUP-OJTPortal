...
try {
    $stmt = $conn->prepare("
        ...
        WHERE 
            student_id = ? AND document_type = 'resume'
        ORDER BY
            upload_date DESC
        LIMIT 1
    ");
    ...
    error_log("Resume fetch error: " . $e->getMessage());
    $resume = null;
}
?>
            ...
            <div class="content-holder">
                <div class="file-overview-holder">
                    <fieldset>
                        ...
                        <div class="overview-cont">
                            <?php if ($resume): ?>
                                <div class="moa-preview" id="resumePreview">
                                    <?php
                                    $file_ext = strtolower(pathinfo($resume['uploaded_path'], PATHINFO_EXTENSION));
                                    if ($file_ext === 'pdf'): ?>
                                        <iframe src="<?php echo $resume['uploaded_path'] . '?t=' . time(); ?>" frameborder="0"></iframe>
                                    <?php elseif (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                        <img src="<?php echo $resume['uploaded_path']; ?>" alt="MOA Image">
                                    <?php else: ?>
                ...
                <div class="info-function-cont">
                        ...
                        <div class="moa-overview">
                            <?php if ($resume): ?>
                                <div class="data-row">
                                    ...
                                    <div class="student-data">: <?php echo htmlspecialchars($resume['document_name']); ?></div>
                                </div>

                                <div class="data-row">
                                    <div class="label"><p><b>Status</b></p></div>
                                    <div class="student-data">: <?php echo htmlspecialchars(ucfirst($resume['status'])); ?></div>
                                </div>
                            <?php else: ?>
                                <div class="data-row">
                    ...
                    <div class="button-function">
                        <button class="btn-style" onclick="viewMoa()" <?php echo $resume ? '' : 'disabled'; ?>>
                            <i class="fa-regular fa-eye"></i> View
                        </button>
                        <button class="btn-style" onclick="updateStatus(<?php echo $resume['document_id'] ?? 'null'; ?>, 'accepted')" <?php echo $resume && $resume['status'] !== 'accepted' ? '' : 'disabled'; ?>>
                            <i class="fa-solid fa-circle-check"></i> Approve
                        </button>
                        <button class="btn-style" onclick="toggleReject()" <?php echo $resume && $resume['status'] !== 'denied' ? '' : 'disabled'; ?>>
...
<script>
    ...
    function viewMoa() {
        <?php if ($resume): ?>
            ...
            if (fileExt === 'pdf') {
                resumePreview.classList.toggle('enlarged');
    ...
    function updateStatus(documentId, status, remarks = null) {
        if (documentId === null) {
            swal("Error", "No Resume document available to update.", "error");
            return;
        }
        swal({
            title: `Are you sure?`,
            text: `You are about to mark this Resume as ${status}.`,
            type: "warning",
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        remarksBtn.addEventListener('click', function() {
            const documentId = <?php echo $resume['document_id'] ?? 'null'; ?>;
        ...
        overlay.addEventListener('click', function () {
            const resumePreview = document.getElementById('resumePreview');
            resumePreview.classList.remove('enlarged');
