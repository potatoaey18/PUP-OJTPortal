...
if (!isset($_SESSION['auth_user']['supervisor_id'])) {
    ...
    sendJsonResponse([
        'status' => 'error',
        'message' => 'Unauthorized: Supervisor not authenticated',
...
try {
    ...
    // Verify the student exists and is assigned to this supervisor
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM conversations WHERE student_id = :student_id AND supervisor_id = :supervisor_id");
        $stmt->bindParam(':student_id', $studentId, PDO::PARAM_INT);
        ...
        if (!$conversation) {
            ...
            $stmt = $conn->prepare("INSERT INTO conversations (student_id, supervisor_id) VALUES (:student_id, :supervisor_id)");
            $stmt->bindParam(':student_id', $studentId, PDO::PARAM_INT);
            $stmt->bindParam(':supervisor_id', $supervisorId, PDO::PARAM_INT);
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $message = [
                ...
                'email' => $row['sender_type'] === 'student' ? ($row['student_email'] ?? null) : ($row['supervisor_email'] ?? null),
        ...
        if (!empty($unreadMessageIds)) {
            ...
            $updateQuery = "UPDATE messages SET is_read = 1 WHERE id IN ($placeholders)";
