...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM conversations_coordinator_student WHERE student_id = ? AND coordinator_id = ?");
    $stmt->execute([$student_id, $coordinator_id]);
    $conversation = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$conversation) {
        $stmt = $conn->prepare("INSERT INTO conversations_coordinator_student (student_id, coordinator_id, created_at) VALUES (?, ?, NOW())");
        $stmt->execute([$student_id, $coordinator_id]);
    ...
    // First, get the coordinator's email from the coordinators_account table using coordinator_id
    ...
    // Get the coordinator's user_id from the users table using their email
    $stmt = $conn->prepare("SELECT user_id FROM users_coordinator_student WHERE email = ? AND user_type = 'coordinator'");
    $stmt->execute([$coordinator_email]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$user) {
        throw new Exception("Supervisor not found in users_coordinator_student table with email: " . $coordinator_email);
    }
    
    $user_id = $user['user_id'];
    
    // Insert message using the user_id from the users table
    $stmt = $conn->prepare("INSERT INTO messages_coordinator_student (conversation_id, sender_id, sender_type, content, created_at) VALUES (?, ?, 'coordinator', ?, NOW())");
    ...
    if (defined('PUSHER_APP_KEY') && !empty(PUSHER_APP_KEY)) {
        try {
            ...
            // Also trigger to coordinator's channel for sync across devices
            $pusher->trigger(
