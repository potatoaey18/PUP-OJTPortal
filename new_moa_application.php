...
// Handle AJAX company info submission
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_company_info') {
    ...
    // Validation
    if (empty($company_name) || empty($nature_of_business) || empty($company_address)) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM new_moa_processing WHERE student_id = ?");
        ...
        if ($existing_moa) {
            ...
            $stmt = $conn->prepare("
                UPDATE new_moa_processing 
                SET company_name = ?, nature_of_business = ?, company_address = ?, 
                    status = 'checking_info', updated_at = NOW()
                WHERE id = ?
            ");
            $stmt->execute([$company_name, $nature_of_business, $company_address, $existing_moa['id']]);
        } else {
            // Insert new record
            $stmt = $conn->prepare("
                INSERT INTO new_moa_processing 
                (student_id, company_name, nature_of_business, company_address, status, request_date, created_at, updated_at)
                VALUES (?, ?, ?, ?, 'checking_info', NOW(), NOW(), NOW())
            ");
            $stmt->execute([$student_id, $company_name, $nature_of_business, $company_address]);
        }

        $response['success'] = true;
        $response['message'] = 'Company information submitted successfully.';
    } catch (PDOException $e) {
        error_log("Company info submission error: " . $e->getMessage());
...
if ($existing_moa) {
    $has_company_info = !empty($existing_moa['company_name']) && 
                        !empty($existing_moa['nature_of_business']) && 
                        !empty($existing_moa['company_address']);
}
?>
...
<body>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0;">
            ...
            <div>
                ...
                <?php else: ?>
                    ...
                    <div class="template-box">
                        <a href="templates/endorsement/New_MOA_Template.docx" download="New_MOA_Template.docx" class="btn1-download-template" style="top: 425px;"><img src="images/doc.png" alt="" class="doc-icon" ><u>New_MOA_Template.docx</u></a>
                        <a href="templates/endorsement/New_MOA_Template.docx" download="New_MOA_Template.docx" class="btn2-download-template btn-download" style="top: 425px;">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="companyInfoModalLabel">Enter Company Information</h5>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="companyInfoForm">
                        <div class="form-group">
                            ...
                            <input type="text" id="company_name" name="company_name" class="form-control" aria-label="Company Name">
                            <div class="error-message" id="company_name_error"></div>
                        </div>
                        <div class="form-group">
                            <label for="nature_of_business">Nature of Business</label>
                            <input type="text" id="nature_of_business" name="nature_of_business" class="form-control" aria-label="Nature of Business">
                            <div class="error-message" id="nature_of_business_error"></div>
                        </div>
                        <div class="form-group">
                            <label for="company_address">Company Address</label>
                            <textarea id="company_address" name="company_address" class="form-control" aria-label="Company Address"></textarea>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const companyInfoModal = $('#companyInfoModal');
        const companyInfoForm = document.getElementById('companyInfoForm');

        // Check if company info exists
        const hasCompanyInfo = <?php echo json_encode($has_company_info); ?>;
        ...
        if (companyInfoForm) {
            companyInfoForm.addEventListener('submit', function(e) {
                ...
                const natureOfBusiness = document.getElementById('nature_of_business').value.trim();
                ...
                if (!natureOfBusiness) {
                    document.getElementById('nature_of_business_error').textContent = 'Nature of business is required.';
                    document.getElementById('nature_of_business_error').style.display = 'block';
                    hasError = true;
                } else {
                    document.getElementById('nature_of_business_error').style.display = 'none';
                }
                ...
                // AJAX submission
                $.ajax({
                    url: 'new_moa_application.php',
                    type: 'POST',
                    data: {
                        action: 'submit_company_info',
                    ...
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                ...
                                fileInput.click(); // Proceed to file upload
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
                    ...
                    error: function() {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'An error occurred while submitting the company information.'
                        });
                    }
                });
            });
    ...
    ?>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                try {
                    const title = "<?php echo isset($_SESSION['alert']) ? addslashes($_SESSION['alert']) : ($_SESSION['status-code'] === 'success' ? 'Success' : ($_SESSION['status-code'] === 'warning' ? 'Warning' : 'Error')); ?>";
                    ...
                    } else if ("<?php echo $_SESSION['status-code']; ?>" === "warning") {
                        if (typeof showConfirmAlert === 'function') {
                            console.log("Calling showConfirmAlert with title: " + title + ", message: " + message);
                            showConfirmAlert({
                                title: title,
                                text: message,
                            ...
                            console.warn("showConfirmAlert is not defined. Falling back to native alert.");
