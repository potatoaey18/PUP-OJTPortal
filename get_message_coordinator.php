...
if (!isset($_SESSION['auth_user']['student_id'])) {
    error_log('Unauthorized in get__coordinator.php: Session=' . print_r($_SESSION, true));
    sendJsonResponse([
        'status' => 'error',
...
try {
    ...
    // Verify the coordinator exists and is assigned to this supervisor
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM conversations_coordinator_student WHERE coordinator_id = :coordinator_id AND student_id = :student_id");
        $stmt->bindParam(':coordinator_id', $coordinatorId, PDO::PARAM_INT);
        ...
        if (!$conversation) {
            ...
            $stmt = $conn->prepare("INSERT INTO conversations_coordinator_student (coordinator_id, student_id) VALUES (:coordinator_id, :student_id)");
            $stmt->bindParam(':coordinator_id', $coordinatorId, PDO::PARAM_INT);
            $stmt->bindParam(':student_id', $studentId, PDO::PARAM_INT);
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $message = [
                ...
                'email' => $row['sender_type'] === 'coordinator' ? ($row['coordinators_email'] ?? null) : ($row['stud_email'] ?? null),
