...
try {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM conversations_coordinator_hte WHERE coordinator_id = :coordinator_id AND supervisor_id = :supervisor_id");
        $stmt->bindParam(':coordinator_id', $coordinatorId, PDO::PARAM_INT);
        ...
        if (!$conversation) {
            ...
            $stmt = $conn->prepare("INSERT INTO conversations_coordinator_hte (coordinator_id, supervisor_id) VALUES (:coordinator_id, :supervisor_id)");
            $stmt->bindParam(':coordinator_id', $coordinatorId, PDO::PARAM_INT);
            $stmt->bindParam(':supervisor_id', $supervisorId, PDO::PARAM_INT);
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $message = [
                ...
                'email' => $row['sender_type'] === 'coordinator' ? ($row['coordinators_email'] ?? null) : ($row['supervisor_email'] ?? null),
        ...
        if (!empty($unreadMessageIds)) {
            ...
            $updateQuery = "UPDATE messages_coordinator_supervisor SET is_read = 1 WHERE id IN ($placeholders)";
