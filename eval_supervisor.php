...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/eval_supervisor/";
$relative_dir = "/PUP/student/uploads/eval_supervisor/";
...
if (isset($_POST['submit'])) {
    ...
    $original_name = basename($_FILES["eval_supervisorDocument"]["name"]);
    ...
    error_log("Temporary file path: " . $_FILES["eval_supervisorDocument"]["tmp_name"]);
    error_log("Upload error code: " . $_FILES["eval_supervisorDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["eval_supervisorDocument"]["tmp_name"]);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if (!in_array($mime, $allowed_mimes)) {
        ...
        header("Location: eval_supervisor.php");
        exit;
    }

    if ($_FILES["eval_supervisorDocument"]["size"] > 10000000) {
        error_log("File size exceeds limit: " . $_FILES["eval_supervisorDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["eval_supervisorDocument"]["tmp_name"], $target_file)) {
        error_log("File successfully moved to: " . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'eval_supervisor'");
        ...
        error_log("File move failed. Error: " . $_FILES["eval_supervisorDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["eval_supervisorDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'eval_supervisor'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Supervisor Evaluation has been approved.",
        'denied' => "Your Supervisor Evaluation was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Supervisor Evaluation has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Supervisor Evaluation</title>
...
<body>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                <div>
                    <a href="evaluations.php" class="back-button">
                ...
                <div class="page-header">
                        <div class="page-title">
                            <h1 style="font-size: 16px;"><b>Evaluate Training Supervisor</b></h1>
                        </div>
                    </div>
                ...
                <div class="instructions">
                    <p>
                        Reminder: Be honest and transparent in your evaluation, as it will be used to assess whether your supervisor meets our university’s standards and expectations for guiding future On-the-Job Training students.
                    </p>
                </div>

                <div>
                    <h1 class="title">Evaluate Supervisor</h1>
                </div>
                
                <div class="template-box">
                    <a href="templates/evaluations/Supervisor_Evaluation_Template.docx" download="Supervisor_Evaluation_Template.docx" class="btn1-download-template" style="margin-top: 105px;"><img src="images/doc.png" alt="" class="doc-icon"><u>Supervisor_Evaluation_Template.docx</u></a>
                    <a href="templates/evaluations/Supervisor_Evaluation_Template.docx" download="Supervisor_Evaluation_Template.docx" class="btn2-download-template btn-download" style="margin-top: 105px;">
                    ...
                    <div class="content">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="eval_supervisorDocument" accept=".pdf,.doc,.docx" class="hidden">
