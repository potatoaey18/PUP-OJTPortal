...
$stmt = $conn->prepare("SELECT sd.* FROM students_data sd 
                      INNER JOIN deployed_students ds ON sd.id = ds.student_id 
                      WHERE ds.company_name = ?");
$stmt->execute([$companyName]);
$results = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Get current supervisor's profile picture
$sql = $conn->prepare("SELECT supervisor_profile_picture FROM supervisor WHERE id = ?");
$sql->execute([$supervisorId]);
$row = $sql->fetch(PDO::FETCH_ASSOC);
$currentImagePath = $row['supervisor_profile_picture'] ?? 'images/default_profile.png';
