...
function logError($message) {
    error_log('[Medical Private Upload] ' . $message);
}
...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/medical/";
$relative_dir = "/PUP/student/uploads/medical/";
...
if (isset($_POST['submit'])) {
    if (!file_exists($target_dir)) {
        if (!mkdir($target_dir, 0755, true)) {
            ...
            header("Location: medical-private.php");
            exit;
        }
        logError("Created directory: " . $target_dir);
    }

    if (!isset($_FILES["medicalDocument"]) || $_FILES["medicalDocument"]["error"] !== UPLOAD_ERR_OK) {
        logError("File upload error: " . ($_FILES["medicalDocument"]["error"] ?? 'No file uploaded'));
    ...
    $original_name = basename($_FILES["medicalDocument"]["name"]);
    ...
    logError("Temporary file path: " . $_FILES["medicalDocument"]["tmp_name"]);
    logError("Upload error code: " . $_FILES["medicalDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["medicalDocument"]["tmp_name"]);
    finfo_close($finfo);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if ($_FILES["medicalDocument"]["size"] > 10000000) {
        logError("File size exceeds limit: " . $_FILES["medicalDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["medicalDocument"]["tmp_name"], $target_file)) {
        logError("File successfully moved to: " . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'medical'");
            ...
            $sql = $existing_document ? 
                "UPDATE endorsement_documents 
                 SET document_name = ?, uploaded_path = ?, upload_date = NOW(), status = ?, medical_at_campus = 'no'
                 WHERE id = ?" :
            ...
            if ($stmt->execute($params)) {
                ...
                $_SESSION['status'] = "Medical certificate uploaded successfully. Awaiting review.";
        ...
        logError("File move failed. Error: " . $_FILES["medicalDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["medicalDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'medical' AND medical_at_campus = 'no'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Medical Certificate has been approved.",
        'denied' => "Your Medical Certificate was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Medical Certificate has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Medical - Private Facility</title>
    ...
    <style>
        .btn-download { display: block; margin-bottom: 1rem; }
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">Medical Certificate Template</h1>
                </div>

                <div class="template-box">
                    <a href="templates/endorsement/medical_template.docx" download="medical_template.docx" class="btn1-download-template">
                        <img src="images/doc.png" alt="Medical Template" class="doc-icon"><u>Medical_Template.docx</u>
                    </a>
                    <a href="templates/endorsement/medical_template.docx" download="medical_template.docx" class="btn2-download-template btn-download">
                    ...
                    <div class="content">
                        <div class="document-area" id="documentArea">
                            <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                                ...
                                ?>
                                    <img id="documentImage" src="<?php echo htmlspecialchars($existing_document['uploaded_path']) . $cache_buster; ?>" alt="Medical Certificate Image">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="medicalDocument" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden">
                                ...
                                <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                                    ...
                                    <div class="notifs-container">
                                        ...
                                        <?php if (!empty($document_status)): ?>
                                            <div class="status-notification <?php echo htmlspecialchars($existing_document['status']); ?>" id="statusNotification" style="display: block;">
                ...
                editBtn.addEventListener('click', function() {
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This medical certificate has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This medical certificate has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
                ...
                <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                    ...
                    if (fileExt === 'pdf') {
                        ...
                        printWindow.document.write(`
                            ...
                            <html>
                            <head>
                                <title>Print Medical Certificate</title>
                                <style>
