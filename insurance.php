...
if (!isset($_SESSION['auth_user']['student_id']) || $_SESSION['auth_user']['student_id'] == 0) {
    error_log('[Insurance Upload] Unauthorized access attempt');
    header('Location: ../pending/login.php');
    exit;
}

$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/insurance/";
$relative_dir = "/PUP/student/uploads/insurance/";
...
if (isset($_POST['submit'])) {
    ...
    if (!file_exists($target_dir)) {
        if (!mkdir($target_dir, 0755, true)) {
            error_log('[Insurance Upload] Failed to create upload directory: ' . $target_dir);
        ...
        error_log('[Insurance Upload] Created directory: ' . $target_dir);
    }

    if (!isset($_FILES["insuranceDocument"]) || $_FILES["insuranceDocument"]["error"] !== UPLOAD_ERR_OK) {
        error_log('[Insurance Upload] File upload error: ' . ($_FILES["insuranceDocument"]["error"] ?? 'No file uploaded'));
    ...
    $original_name = basename($_FILES["insuranceDocument"]["name"]);
    $file_name = $original_name;
    $target_file = $target_dir . $student_id . "_" . $file_name;
    $relative_path = $relative_dir . $student_id . "_" . $file_name;

    error_log('[Insurance Upload] Original file name: ' . $original_name);
    error_log('[Insurance Upload] Target file path: ' . $target_file);
    error_log('[Insurance Upload] Relative path: ' . $relative_path);
    error_log('[Insurance Upload] Temporary file path: ' . $_FILES["insuranceDocument"]["tmp_name"]);
    error_log('[Insurance Upload] Upload error code: ' . $_FILES["insuranceDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["insuranceDocument"]["tmp_name"]);
    finfo_close($finfo);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if (!in_array($mime, $allowed_mimes)) {
        error_log('[Insurance Upload] Invalid MIME type: ' . $mime);
    ...
    if ($_FILES["insuranceDocument"]["size"] > 10000000) {
        error_log('[Insurance Upload] File size exceeds limit: ' . $_FILES["insuranceDocument"]["size"]);
    ...
    if (file_exists($target_file)) {
        error_log('[Insurance Upload] Existing file deleted: ' . $target_file);
        unlink($target_file);
    }

    if (move_uploaded_file($_FILES["insuranceDocument"]["tmp_name"], $target_file)) {
        error_log('[Insurance Upload] File successfully moved to: ' . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'insurance'");
            ...
            if ($stmt->execute($params)) {
                error_log('[Insurance Upload] Database updated successfully. Path: ' . $relative_path);
                $_SESSION['status'] = "Insurance document uploaded successfully. Awaiting review.";
                $_SESSION['status-code'] = "success";
                $_SESSION['alert'] = "Success";
            } else {
                error_log('[Insurance Upload] Database update failed.');
                $_SESSION['status'] = "Failed to save document details.";
                $_SESSION['status-code'] = "error";
                $_SESSION['alert'] = "Error";
            }
        } catch (PDOException $e) {
            error_log('[Insurance Upload] Database error: ' . $e->getMessage());
            $_SESSION['status'] = "System error. Contact support.";
            $_SESSION['status-code'] = "error";
            $_SESSION['alert'] = "Error";
        }
    } else {
        error_log('[Insurance Upload] File move failed. Error: ' . $_FILES["insuranceDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["insuranceDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'insurance'");
    $stmt->execute([$_SESSION['auth_user']['student_id']]);
    $existing_document = $stmt->fetch(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log('[Insurance Upload] Document fetch error: ' . $e->getMessage());
    $existing_document = null;
}

if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Insurance document has been approved.",
        'denied' => "Your Insurance document was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Insurance document has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Insurance</title>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">Insurance Document Submission</h1>
                </div>

                <div class="template-box">
                    <a href="templates/endorsement/insurance_template.docx" download="insurance_template.docx" class="btn1-download-template"><img src="images/doc.png" alt="" class="doc-icon"><u>Insurance_Template.docx</u></a>
                    <a href="templates/endorsement/insurance_template.docx" download="insurance_template.docx" class="btn2-download-template btn-download">
                        <i class="fa fa-download"></i>
                    </a>
                </div>

                <div>
                    <h1 class="title">Insurance Document</h1>
                </div>
                    ...
                    <div class="content">
                        <div class="document-area" id="documentArea">
                            <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                                ...
                                ?>
                                    <img id="documentImage" src="<?php echo htmlspecialchars($existing_document['uploaded_path']) . $cache_buster; ?>" alt="Insurance Image">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="insuranceDocument" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden">
                ...
                editBtn.addEventListener('click', function() {
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This insurance document has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This insurance document has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
