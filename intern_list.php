...
<head>
    ...
    <style>
        ...
        .table td:nth-child(1) { /* Date Started */
            min-width: 130px;
        }

        .table td:nth-child(2) { /* Hours Left */
            min-width: 130px;
        }
...
<body>
    ...
    <div class="content-wrap">
        ...
        <div class="page-title">
            <h1 style="font-size: 16px;"><b>Intern List</b></h1>
        </div>
...
foreach ($departments as $normalized_department) {
    ...
    echo '<tr class="column-headers">';
                echo '<td style="width: 12%;">Date Started</td>';
                echo '<td style="width: 10%;">Hours Left</td>';
                echo '<td style="width: 25%;">Full Name</td>';
                echo '<td style="width: 10%;">Status</td>';
                echo '<td style="width: 12%;">View DTR</td>';
                echo '<td style="width: 40%; text-align: center;">Actions</td>';
                echo '</tr>';
                $found = false;
    foreach ($students as $student) {
        ...
        if ($deployment && strtoupper(trim($deployment['department'])) === $normalized_department) {
                        ...
                        $date_started = isset($student['date_started']) ? htmlspecialchars($student['date_started']) : '-';
                        $target_hours = isset($student['target_hours']) ? htmlspecialchars($student['target_hours']) : '480';
                        $hours_completed = isset($student['hours_completed']) ? htmlspecialchars($student['hours_completed']) : '0';
                        $status = htmlspecialchars($student['ojt_status']);
                        echo '<tr class="data-row">';
                        echo '<td>' . $date_started . '</td>';
                        echo '<td>' . ($target_hours - $hours_completed) . '</td>';
                        echo '<td>' . $full_name . '</td>';
                        echo '<td>' . $status . '</td>';
                        echo '<td><a href="view_dtr.php?student_id=' . urlencode($student['student_ID']) . '" class="view-dtr-btn">View DTR</a></td>';
                        echo '<td class="action-buttons">';
                        echo '<button class="view-profile-btn" data-intern-id="' . htmlspecialchars($student['student_ID']) . '">View Profile</button>';
                        echo '<button class="drop-btn" data-intern-id="' . htmlspecialchars($student['student_ID']) . '">Drop</button>';
                        echo '<button class="complete-btn" data-intern-id="' . htmlspecialchars($student['student_ID']) . '">Complete</button>';
                        echo '</td>';
                        echo '</tr>';
                    }
                }
                                ...
                                <div style="padding: 12px 20px 20px 20px; border-radius: 12px; background-color: #f8f9fa; margin-top: -18px;">
                                    <div class="form-group" style="margin-bottom: 25px;">
                                        <label style="display: block; text-align: left; margin-bottom: 8px; font-weight: bold; font-size: 14px;">
                                            Please provide a reason for dropping <span class="student-name-highlight1">${fullName}</span>:
                                        ...
                                        <div style="display: flex; justify-content: center;">
                                            <textarea
                                                ...
                                                placeholder="Ex. Irresponsible with his/her tasks."
    ...
    .then(data => {
        if (data.status === 'success') {
            Swal.fire({
                                            icon: 'success',
                                            title: '<span style="margin-top: 25px; display: block;">Student Dropped</span>',
                ...
                intern_id: internId
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire({
    icon: 'success',
    title: '<span style="margin-top: 25px; display: block;">Marked as Completed</span>',
    html: '<p style="font-size: 12px; color: white; margin-top: -15px; opacity: 0.5; font-style: italic;">(Click anywhere to continue)</p>',
    showConfirmButton: false,
    background: '#700000',
    color: 'white',
    ...
    $(document).on('click', '.view-dtr-btn', function(e) {
        e.preventDefault();
        const studentId = $(this).closest('tr').find('.view-profile-btn').data('intern-id');
        window.location.href = 'view_dtr1.php?student_id=' + encodeURIComponent(studentId);
    });
});
