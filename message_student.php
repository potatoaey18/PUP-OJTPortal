...
<body>
        ...
        <div class="profile-container">
            <div class="messaging-container">
                ...
                <div class="contacts-panel">
                    <?php
                    // Fetch coordinator's details from the database
                    $coordinator_id = $_SESSION['auth_user']['coordinators_id'] ?? 0;
                    $coordinator_name = 'Coordinator';
                    $profile_picture = 'default-avatar.png'; // Default avatar if none is set
                    
                    if ($coordinator_id) {
                        $stmt = $conn->prepare("SELECT first_name, middle_name, last_name, coordinators_profile_picture FROM coordinators_account WHERE id = :id");
                        $stmt->execute([':id' => $coordinator_id]);
                        $coordinator = $stmt->fetch(PDO::FETCH_ASSOC);
                        if ($coordinator) {
                            $coordinator_name = htmlspecialchars($coordinator['first_name'] . ' ' . ($coordinator['middle_name'] ? $coordinator['middle_name'] . ' ' : '') . $coordinator['last_name']);
                            if (!empty($coordinator['coordinators_profile_picture'])) {
                                $profile_picture = $coordinator['coordinators_profile_picture'];
                            }
                        }
                    }
                    ?>
                    ...
                    <div class="contacts-list">
                        ...
                        $coordinator_id = $_SESSION['auth_user']['coordinators_id'] ?? 0;
                        try {
                            $stmt = $conn->prepare("SELECT * FROM students_data ORDER BY first_name, last_name");
                            $stmt->execute();
                            ...
                            foreach ($students as &$student) {
                                $stmt = $conn->prepare("
                                    ...
                                    LIMIT 1
                                ");
                                $stmt->execute([$coordinator_id, $student['id']]);
                                ...
                                if ($conversation) {
                                    ...
                                    $student['last_message'] = $lastMessage ? $lastMessage['content'] : '';
                        ...
                        if (count($students) > 0) {
                            foreach ($students as $student) {
                                ...
                                $status = $student['online_offlineStatus'] === 'Online' ? 'online' : 'offline';
                                $lastMessage = '';
                                if (!empty($student['last_message'])) {
                                    $lastMessage = htmlspecialchars($student['last_message']);
                                    if (strlen($lastMessage) > 30) {
                                        $lastMessage = substr($lastMessage, 0, 27) . '...';
                                    }
                                }
                                
                                echo "<div class='contact-item' data-student-id='{$student['id']}' data-student-course='".htmlspecialchars($student['stud_course'] ?? 'No course specified')."'>";
                                ...
                                echo "<span class='preview-text' title='".htmlspecialchars($student['last_message'] ?? '')."'>{$lastMessage}</span>";
                                echo "</div></div></div>";
                            }
                        } else {
                            echo "<div class='text-center p-4'>No students found</div>";
                        }
                        ?>
                    </div>
                </div>
    ...
    $(document).ready(function() {
        let currentStudentId = null; // Global variable for current student ID

        // Function to save messages to localStorage
        function saveMessages(studentId, messages) {
            localStorage.setItem(`messages_${studentId}`, JSON.stringify(messages));
        }

        // Function to load messages from localStorage
        function loadSavedMessages(studentId) {
            const saved = localStorage.getItem(`messages_${studentId}`);
        ...
        function sortContactsByLastMessage() {
            ...
            $contacts.sort((a, b) => {
                const aId = $(a).data('student-id');
                const bId = $(b).data('student-id');
        ...
        function updateContactPreview(studentId, message, isNewMessage = true, previewTime = null) {
            console.log('Updating contact preview for student:', studentId, 'Message:', message, 'isNewMessage:', isNewMessage, 'previewTime:', previewTime);
            
            const contactItem = document.querySelector(`.contact-item[data-student-id="${studentId}"]`);
            if (!contactItem) {
                console.error('Contact item not found for student ID:', studentId);
                return null;
            }
            ...
            previews[studentId] = previewData;
        ...
        function renderMessages(messages) {
            ...
            messages.forEach(msg => {
                // Check if the message was sent by the current user (coordinator)
                // First check for sender_type, then fall back to msg.sender === 'me' or msg.sender === 'coordinator'
                const isMe = msg.sender_type === 'coordinator' || 
                           msg.sender === 'me' || 
                           (msg.sender && msg.sender.includes('coordinator'));
                
                const decodedText = decodeHtmlEntities(msg.text || msg.content || '');
                const messageTime = msg.time || (msg.created_at ? formatTime(new Date(msg.created_at)) : new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
                
                const messageHtml = `
                    <div class="message ${isMe ? 'sent' : 'received'}" data-message-id="${msg.id || ''}">
                        <div class="message-content">
                            <div class="message-text">${decodedText}</div>
            ...
            if (messages.length > 0 && currentStudentId) {
                ...
                updateContactPreview(currentStudentId, lastMessage.text, false);
            }
        ...
        function loadStudentMessages(studentId) {
            ...
            $(`.contact-item[data-student-id="${studentId}"]`).addClass('active');
            
            const contactItem = $(`.contact-item[data-student-id="${studentId}"]`);
            const studentName = contactItem.find('.contact-name').text().trim().split(' ')[0] + ' ' + contactItem.find('.contact-name').text().trim().split(' ')[1];
            $('#chatStudentName').text(studentName || 'Student');
            ...
            const studentCourse = contactItem.data('student-course') || 'No course specified';
            $('#chatStudentInfo').text(studentCourse);
            ...
            const savedMessages = loadSavedMessages(studentId);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success' && response.data && Array.isArray(response.data.messages)) {
                        ...
                        saveMessages(studentId, formattedMessages);
        ...
        function markMessagesAsRead(studentId) {
            ...
            $('.message[data-student-id="' + studentId + '"]').removeClass('unread').addClass('read');
            updateUnreadCount(studentId, false);
            
            const unreadMessages = [];
            $('.message[data-student-id="' + studentId + '"].unread').each(function() {
        ...
        function updateUnreadCount(studentId, increment = false) {
            const contactItem = $(`.contact-item[data-student-id="${studentId}"]`);
        ...
        function sendMessage() {
            ...
            console.log('Message to send:', message, ', to student:', currentStudentId);
            
            if (!message || !currentStudentId) {
                console.log('Message or student ID is missing');
                return;
            }
    ...
    if (typeof Toastify !== 'undefined') {
        ...
        function loadContactPreviews() {
            ...
            $('.contact-item').each(function() {
                ...
                const previewData = previews[studentId];
        ...
        function selectConversation(studentId, studentName) {
            currentStudentId = studentId;
            $('#currentStudentId').val(studentId);
            
            $('.contact-item').removeClass('active');
            $(`[data-student-id="${studentId}"]`).addClass('active');
            
            const contactItem = $(`[data-student-id="${studentId}"]`);
            const status = contactItem.find('.contact-status').text().trim();
            const program = 'BS Computer Engineering'; // Default program
            
            $('#chatAvatar').text(studentName.split(' ').map(n => n[0]).join('').toUpperCase());
            $('#chatStudentName').text(studentName);
        ...
        function initializePusher() {
            ...
            try {
                ...
                const coordinatorId = '<?php echo isset($_SESSION['auth_user']['coordinators_id']) ? $_SESSION['auth_user']['coordinators_id'] : '0'; ?>';
                
                if (!appKey || !cluster || !coordinatorId || coordinatorId === '0') {
                    console.error('Pusher initialization failed - missing credentials', { appKey, cluster, coordinatorId });
                ...
                const channel = pusher.subscribe('private-coordinator-' + coordinatorId);
                
                channel.bind('new_message', function(data) {
                    console.log('New message received:', data);
                    if (data.sender_type === 'student') {
                        const messageId = data.id || 'msg_' + Date.now();
                        const isMe = data.sender_id === '<?php echo isset($_SESSION['auth_user']['coordinators_id']) ? $_SESSION['auth_user']['coordinators_id'] : '0'; ?>';
                        ...
                        const senderId = data.sender_id || data.student_id;
                        if (!currentStudentId && senderId) {
                            currentStudentId = senderId;
                            $('#currentStudentId').val(senderId);
                        }
                        ...
                        if (!messageExists) {
                            ...
                            updateContactPreview(senderId || currentStudentId, data.content, true, previewTime);
                            
                            if (senderId !== currentStudentId) {
                                showToast('New Message', `New message from student ${senderId}`, 'info');
                ...
                channel.bind('pusher:subscription_succeeded', function() {
                    console.log('Successfully subscribed to channel: private-coordinator-' + coordinatorId);
                ...
                channel.bind('pusher:subscription_error', function(status, response) {
                    console.error('Pusher subscription error:', {
                        ...
                        channel: 'private-coordinator-' + coordinatorId
                    });
                    showToast('Error', `Failed to establish real-time messaging connection (Status: ${status}).`, 'error');
                });
                
                console.log('Pusher initialized for channel: private-coordinator-' + coordinatorId);
                return pusher;
            } catch (error) {
        ...
        $(document).on('click', '.contact-item', function() {
            const studentId = $(this).data('student-id');
            const studentName = $(this).find('.contact-name').text().trim();
            if (studentId) {
                selectConversation(studentId, studentName);
            }
        });
