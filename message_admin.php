...
error_log("Auth user data: " . print_r($_SESSION['auth_user'] ?? 'No auth_user in session', true));
...
if (!isset($_SESSION['auth_user']['coordinators_id'])) {
    error_log("coordinators_id not found in session. Redirecting to index.php");
...
<body>
            ...
            <div class="messaging-container">
                ...
                <div class="contacts-panel">
                    ...
                    <div class="contacts-list">
                        ...
                        $coordinator_id = $_SESSION['auth_user']['coordinators_id'] ?? 0;
                        try {
                            $stmt = $conn->prepare("SELECT * FROM admin_account ORDER BY first_name, last_name");
                            $stmt->execute();
                            ...
                            foreach ($admins as &$admin) {
                                $stmt = $conn->prepare("
                                    SELECT c.id 
                                    FROM conversations_admin_coordinator c
                                    JOIN conversation_participants cp1 ON c.id = cp1.conversation_id
                                    JOIN conversation_participants cp2 ON c.id = cp2.conversation_id
                                    WHERE cp1.user_id = ? AND cp2.user_id = ?
                                    LIMIT 1
                                ");
                                $stmt->execute([$coordinator_id, $admin['id']]);
                                ...
                                if ($conversation) {
                                    ...
                                    $admin['last_message'] = $lastMessage ? $lastMessage['content'] : '';
                                    $admin['last_message_time'] = $lastMessage ? $lastMessage['created_at'] : null;
                                } else {
                                    $admin['last_message'] = '';
                            ...
                            // Sort the admins array by last message time
                        ...
                        if (count($admins) > 0) {
                            foreach ($admins as $admin) {
                                ...
                                $status = $admin['online_offlineStatus'] === 'Online' ? 'online' : 'offline';
                                $lastMessage = '';
                                if (!empty($admin['last_message'])) {
                                    $lastMessage = htmlspecialchars($admin['last_message']);
                                    if (strlen($lastMessage) > 30) {
                                        $lastMessage = substr($lastMessage, 0, 27) . '...';
                                    }
                                }
                                
                                echo "<div class='contact-item' data-admin-id='{$admin['id']}' data-admin-course='".htmlspecialchars($admin['stud_course'] ?? 'No course specified')."'>";
                                ...
                                echo "<span class='preview-text' title='".htmlspecialchars($admin['last_message'] ?? '')."'>{$lastMessage}</span>";
                                echo "</div></div></div>";
                            }
                        } else {
                            echo "<div class='text-center p-4'>No admin found</div>";
                        }
                        ?>
                    </div>
                </div>
    ...
    $(document).ready(function() {
        ...
        function renderMessages(messages) {
            ...
            if (messages.length > 0 && currentAdminId) {
                const lastMessage = messages[messages.length - 1];
                updateContactPreview(currentAdminId, lastMessage.text, false);
            }
        ...
        function loadAdminMessages(adminId) {
            if (!adminId) return;
            ...
            $('#chatAdminName').text(adminName || 'Admin');
            ...
            const adminCourse = contactItem.data('admin-course') || 'No course specified';
            $('#chatAdminInfo').text(adminCourse);
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success' && response.data && Array.isArray(response.data.messages)) {
                        const formattedMessages = response.data.messages.map(msg => ({
                            ...
                            sender: msg.sender_type === 'coordinator' ? 'me' : 'them',
                            text: msg.content,
        ...
        function sendMessage() {
            ...
            console.log('Message to send:', message, ', to admin:', currentAdminId);
            
            if (!message || !currentAdminId) {
            ...
            const existingMessages = loadSavedMessages(currentAdminId);
            existingMessages.push(newMessage);
            saveMessages(currentAdminId, existingMessages);
            renderMessages(existingMessages);
            updateContactPreview(currentAdminId, message, true);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success' && response.data) {
                        const messages = loadSavedMessages(currentAdminId);
                        ...
                        if (messageIndex !== -1) {
                            ...
                            saveMessages(currentAdminId, messages);
        ...
        function loadContactPreviews() {
            try {
                ...
                $('.contact-item').each(function() {
                    ...
                    if (!adminId) return; // Skip if no admin ID
                    
                    const previewData = previews[adminId];
                    const $previewElement = $this.find('.contact-preview');
                    const $timeElement = $this.find('.contact-time');
                    
                    if (previewData && typeof previewData === 'object') {
                        // Safely handle preview text with fallback
                        const previewText = (previewData.preview || '').toString().trim();
                        const fullMessage = (previewData.fullMessage || '').toString();
                        const previewTime = previewData.time || timeString;
                        
                        $previewElement.text(previewText).attr('title', fullMessage);
                        $timeElement.text(previewTime);
                        hasUpdates = true;
                    } else {
                        // Initialize with default values if no preview data exists
                        ...
                        $previewElement.text(initialMessage);
                ...
                if (hasUpdates) {
                    try {
                        localStorage.setItem('contactPreviews', JSON.stringify(previews));
                        if (typeof sortContactsByLastMessage === 'function') {
                            sortContactsByLastMessage();
                        }
                    } catch (e) {
                        console.error('Error saving previews to localStorage:', e);
                    }
                }
            } catch (error) {
                console.error('Error in loadContactPreviews:', error);
            }
        }
            ...
            try {
                ...
                channel.bind('new_message', function(data) {
                    ...
                    if (data.sender_type === 'admin') {
                        ...
                        const existingMessages = loadSavedMessages(senderId || currentAdminId);
                        const messageExists = existingMessages.some(msg => msg.id === messageId || (msg.text === data.content && msg.sender === (isMe ? 'me' : 'them')));
                        
                        if (!messageExists) {
                            existingMessages.push(newMessage);
                            saveMessages(senderId || currentAdminId, existingMessages);
                            
                            // Update preview time regardless of the current chat
                            updateContactPreview(senderId || currentAdminId, data.content, true, previewTime);
                            ...
                            if (!currentAdminId || senderId === currentAdminId) {
                                renderMessages(existingMessages);
                                if (senderId === currentAdminId) {
