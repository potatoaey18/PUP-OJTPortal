...
function logError($message) {
    error_log('[Internship Agreement Upload] ' . $message);
}
...
$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/PUP/student/uploads/internship_agreement/";
$relative_dir = "/PUP/student/uploads/internship_agreement/";
...
if (isset($_POST['submit'])) {
    ...
    if (!file_exists($target_dir)) {
        if (!mkdir($target_dir, 0755, true)) {
            ...
            header("Location: internship_agreement.php");
            exit;
        }
        logError("Created directory: " . $target_dir);
    }

    if (!isset($_FILES["internshipAgreementDocument"]) || $_FILES["internshipAgreementDocument"]["error"] !== UPLOAD_ERR_OK) {
        logError("File upload error: " . ($_FILES["internshipAgreementDocument"]["error"] ?? 'No file uploaded'));
    ...
    $original_name = basename($_FILES["internshipAgreementDocument"]["name"]);
    ...
    logError("Temporary file path: " . $_FILES["internshipAgreementDocument"]["tmp_name"]);
    logError("Upload error code: " . $_FILES["internshipAgreementDocument"]["error"]);

    $finfo = finfo_open(FILEINFO_MIME_TYPE);
    $mime = finfo_file($finfo, $_FILES["internshipAgreementDocument"]["tmp_name"]);
    finfo_close($finfo);
    $allowed_mimes = [
        'application/pdf', 
        'application/msword', 
    ...
    if ($_FILES["internshipAgreementDocument"]["size"] > 10000000) {
        logError("File size exceeds limit: " . $_FILES["internshipAgreementDocument"]["size"]);
    ...
    if (move_uploaded_file($_FILES["internshipAgreementDocument"]["tmp_name"], $target_file)) {
        logError("File successfully moved to: " . $target_file);
        try {
            $checkStmt = $conn->prepare("SELECT id, status FROM endorsement_documents WHERE student_id = ? AND document_type = 'internship_agreement'");
            ...
            $sql = $existing_document ? 
                ...
                "INSERT INTO endorsement_documents 
                 ...
                 VALUES (?, ?, 'internship_agreement', ?, NOW(), ?)";
            ...
            if ($stmt->execute($params)) {
                ...
                $_SESSION['status'] = "Internship Agreement uploaded successfully. Awaiting review.";
        ...
        logError("File move failed. Error: " . $_FILES["internshipAgreementDocument"]["error"]);
        $_SESSION['status'] = "Upload failed. Error: " . $_FILES["internshipAgreementDocument"]["error"];
        $_SESSION['status-code'] = "error";
        $_SESSION['alert'] = "Error";
    }
...
try {
    $stmt = $conn->prepare("SELECT *, DATE_FORMAT(upload_date, '%M %d, %Y %H:%i') AS formatted_date 
                           ...
                           WHERE student_id = ? AND document_type = 'internship_agreement'");
...
if ($existing_document) {
    $status_msg = [
        'accepted' => "Your Internship Agreement has been approved.",
        'denied' => "Your Internship Agreement was denied. Please review the feedback and submit a revised version.",
        'pending' => "Your Internship Agreement has been submitted and is under review."
    ];
...
<head>
    ...
    <title>OJT Web Portal: Internship Agreement</title>
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto; position: relative;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            <div>
                ...
                <div>
                    <h1 class="moa-title">Internship Agreement Submission</h1>
                </div>

                <div class="template-box">
                    <a href="templates/endorsement/ia.docx" download="ia.docx" class="btn1-download-template"><img src="images/doc.png" alt="" class="doc-icon"><u>Internship_Agreement_Template.docx</u></a>
                    <a href="templates/endorsement/ia.docx" download="ia.docx" class="btn2-download-template btn-download">
                        <i class="fa fa-download"></i>
                    </a>
                </div>

                <div>
                    <h1 class="title">Internship Agreement</h1>
                </div>
                    ...
                    <div class="content">
                        <div class="document-area" id="documentArea">
                            <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                                ...
                                ?>
                                    <img id="documentImage" src="<?php echo htmlspecialchars($existing_document['uploaded_path']) . $cache_buster; ?>" alt="Internship Agreement Image">
                        ...
                        <div class="action-buttons">
                            <form id="uploadForm" action="" method="POST" enctype="multipart/form-data">
                                <input type="file" id="fileInput" name="internshipAgreementDocument" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" class="hidden">
                ...
                editBtn.addEventListener('click', function() {
                    if (documentStatus === 'accepted') {
                        if (typeof showConfirmAlert === 'function') {
                            ...
                            showConfirmAlert({
                                title: "Warning",
                                text: "This Internship Agreement has been accepted. Editing requires resubmission. Continue?",
                            ...
                            if (confirm("This Internship Agreement has been accepted. Editing requires resubmission. Continue?")) {
                                fileInput.click();
                            }
                        }
                    } else {
                ...
                <?php if ($existing_document && !empty($existing_document['uploaded_path'])): ?>
                    ...
                    if (fileExt === 'pdf') {
                        ...
                        printWindow.document.write(`
                            ...
                            <html>
                            <head>
                                <title>Print Internship Agreement</title>
                                <style>
