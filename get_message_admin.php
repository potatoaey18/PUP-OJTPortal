...
try {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM conversations_admin_hte WHERE admin_id = :admin_id AND supervisor_id = :supervisor_id");
        $stmt->bindParam(':admin_id', $adminId, PDO::PARAM_INT);
        ...
        if (!$conversation) {
            ...
            $stmt = $conn->prepare("INSERT INTO conversations_admin_hte (admin_id, supervisor_id) VALUES (:admin_id, :supervisor_id)");
            $stmt->bindParam(':admin_id', $adminId, PDO::PARAM_INT);
            $stmt->bindParam(':supervisor_id', $supervisorId, PDO::PARAM_INT);
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $message = [
                ...
                'email' => $row['sender_type'] === 'admin' ? ($row['admin_email'] ?? null) : ($row['supervisor_email'] ?? null),
