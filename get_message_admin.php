...
if (!isset($_SESSION['auth_user']['student_id'])) {
    error_log('Unauthorized in get__.php: Session=' . print_r($_SESSION, true));
    sendJsonResponse([
        'status' => 'error',
...
try {
    ...
    // Verify the student exists and is assigned to this admin
    $query = "SELECT id FROM admin_account WHERE id = :admin_id";
    error_log("Executing query: $query with admin_id: $adminId");
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM conversations_admin_student WHERE admin_id = :admin_id AND student_id = :student_id");
        $stmt->bindParam(':admin_id', $adminId, PDO::PARAM_INT);
        ...
        if (!$conversation) {
            ...
            $stmt = $conn->prepare("INSERT INTO conversations_admin_student (admin_id, student_id) VALUES (:admin_id, :student_id)");
            $stmt->bindParam(':admin_id', $adminId, PDO::PARAM_INT);
            $stmt->bindParam(':student_id', $studentId, PDO::PARAM_INT);
        ...
        $query = "SELECT 
                    ...
                    CASE 
                        ...
                        WHEN m.sender_type = 'student' AND su.id IS NOT NULL THEN CONCAT(su.first_name, ' ', su.last_name)
                        ELSE 'Unknown Sender'
                    END as sender_name,
                    ad.admin_email as admin_email,
                    su.stud_email as stud_email
                FROM messages_admin_student m
                LEFT JOIN admin_account ad ON ad.id = m.sender_id AND m.sender_type = 'admin'
                LEFT JOIN students_data su ON su.id = m.sender_id AND m.sender_type = 'student'
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $message = [
                ...
                'email' => $row['sender_type'] === 'admin' ? ($row['admin_email'] ?? null) : ($row['stud_email'] ?? null),
            ...
            if ($row['sender_type'] === 'admin' && !$row['is_read']) {
        ...
        if (!empty($unreadMessageIds)) {
            ...
            $updateQuery = "UPDATE messages_admin_student SET is_read = 1 WHERE id IN ($placeholders)";
