...
if (!isset($_SESSION['auth_user']['coordinators_id'])) {
    error_log('Unauthorized in get_messages_admin.php: Session=' . print_r($_SESSION, true));
    sendJsonResponse([
        'status' => 'error',
...
try {
    ...
    // Verify the admin exists and is assigned to this supervisor
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT id FROM conversations_admin_coordinator WHERE admin_id = :admin_id AND coordinator_id = :coordinator_id");
        $stmt->bindParam(':admin_id', $adminId, PDO::PARAM_INT);
        ...
        if (!$conversation) {
            ...
            $stmt = $conn->prepare("INSERT INTO conversations_admin_coordinator (admin_id, coordinator_id) VALUES (:admin_id, :coordinator_id)");
            $stmt->bindParam(':admin_id', $adminId, PDO::PARAM_INT);
            $stmt->bindParam(':coordinator_id', $coordinatorId, PDO::PARAM_INT);
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $message = [
                ...
                'email' => $row['sender_type'] === 'admin' ? ($row['admin_email'] ?? null) : ($row['coordinators_email'] ?? null),
