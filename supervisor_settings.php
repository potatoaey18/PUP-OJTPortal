...
if (isset($_POST['updateInfo'])) {
    ...
    if ($fname !== $currentData['first_name'] ||
        ...
        $c_address !== $currentData['company_address'] ||
        $cp_number !== $currentData['phone_number']) {


        $stmt = $conn->prepare("UPDATE supervisor SET first_name=?, middle_name=?, last_name=?, company_address=?, phone_number=? WHERE id=?");
        $stmt->execute([$fname, $mname, $lname, $c_address, $cp_number, $supervisorID]);
        ...
        if ($stmt->rowCount() > 0) {
          ...
          $sql2->execute([$supervisorID, $logs, $date, $time]);
...
if (isset($_POST['updatePword'])) {
    ...
    $password = $currentData['supervisor_password'];

    if ($pword == $password) {
        if ($npword == $rnpword) {

  
        $stmt = $conn->prepare("UPDATE supervisor SET supervisor_password = ? WHERE id=?");
        $stmt->execute([$npword, $supervisorID]);
...
<head>
    ...
    <title>OJT Web Portal: Settings</title>
    <!-- ================= Favicon ================== -->
    <!-- Standard -->
...
if(isset($_SESSION['auth_user']['userid'])){
  ...
  $studID = $_SESSION['auth_user']['userid'];
...
<form action="" method="POST" class="row g-3 needs-validation" novalidate="">
  <div class="col-md-4 position-relative">
    ...
    <input type="hidden"name="supervisorID"value="<?php echo $data['id']; ?>" required="">
  ...
  <div class="col-md-4 position-relative">
    <label for="validationTooltip02" class="form-label">Company Address</label>
    <input type="text" class="form-control" name="address" id="validationTooltip02" value="<?php echo $data['company_address']; ?>" required="">
    <div class="valid-tooltip">
      Looks good!
    </div>
    <div class="invalid-tooltip">
